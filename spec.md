...
| Skills | `canUseSkill`, `executeSkill`, `learnSkill`, `carryPhilosophicalResources`, `Skill`, `SkillCategory`, `SkillSynergy`, `evaluateExtendedSynergyPredicate`; Phase 142 extended synergy predicate helpers — `checkSinglePredicate`, `checkAnyCountPredicate`, `checkAllRequiredPredicate`, `checkBuffDebuffCombo`, `checkTotalIntensityPredicate`, `ExtendedSynergyPredicate`; resource helpers — `generateBasicActionResources` (adds stance tokens per generation table given stance + outcome), `generatePhilosophicalResource` (adds 1 Fallacy or Paradox token after skill use), `spendResources` (deducts resource cost; guard with `canUseSkill` first), `calculateSkillDamage` (applies the damage formula for a skill + advantage) |
