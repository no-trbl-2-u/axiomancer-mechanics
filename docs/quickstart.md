...
| **Combat** | Hazard-Pattern card/dice engine (`initializeCombatEncounter`, `playCombatCard`, `resolveThreatPhase`, `buildCombatSummary`), hidden-read / Conviction / Signature, mercy via `selectMercyChoice` | 9, 15, 32, 36, 38 | [combat.md](./combat.md) |
...
| **Game** | `createGameStore`, save/load + migrators (`GAME_STATE_VERSION` 10), event surface, autosave throttling, persistence adapters, run-loop semantics (`resetRun` + `runId`), Codex slice | 9, 11, 12, 21, 35, 38, 50, 51, 55, 72, 73 | [gameloop.md](./gameloop.md) |
...
| **World** | `createStartingWorld` + per-continent maps, MapEvents engine (`resolveMapEvent`, ten-kind pool taxonomy — Phase 23/24, 'quest' added Phase 137, 'narration' added later), expanded fishing-village (Phase 65 — 25 nodes, 3 sub-areas) | 8, 23, 24, 25, 31, 65 | [world.md](./world.md) |
...
| Tab | Surface |
|---|---|
| **Map** | Walk available nodes, resolve MapEvents (`resolveMapEvent`), see discovered / consumed nodes. An `encounter` node **stages** combat and prints `npm run combat` — combat is not a tab. |
| **Travel** | Cross to another map on the current continent (e.g. `fishing-village` → `northern-forest`) via `store.travelToMap` |
| **Journal** | Read-only: active / completed quests + flags + alignment |
| **Skills** | Read-only: learned / unlocked skills |
| **Codex** | Read-only render of unlocked journal entries from befriended foes (Phase 73) |
| **Inventory** | Read-only listing of carried items |
| **Character** | Full stats + equipment + effects sheet; allocate stat points (Phase 29), learn skills (Phase 30) |
| **DEV** | Manipulate character, grant items/skills, spawn arbitrary enemies for testing |
| **Begin again** | Reset to the starting hearth via `store.resetRun({ keepCharacter })` (Phase 72) |
...
Combat itself runs in the separate Hazard-Pattern combat CLI
(`npm run combat`); a map `encounter` node only stages it.
...
### Combat turn → mercy path

Combat is the Hazard-Pattern card/dice engine (`src/Combat/`), run
from the standalone `npm run combat` CLI. The enemy's SOLE bar is HP;
status effects are the efficient way to drop it to 0.

1. Enter combat: `initializeCombatEncounter(...)` builds a
   `CombatEncounterState`.
2. Each turn: `rollEncounterDice` / `startTurn` roll the turn's dice,
   `draftStanceDie` picks a stance die, then `playCombatCard` stages
   one or more cards; `resolveThreatPhase` resolves the enemy's
   telegraphed threat and `processBetweenPhases` ticks status effects
   (DoT erodes HP, control denies the enemy its turn).
3. **Victory path** — when `isDefeated(enemy)` (enemy HP → 0), the
   encounter ends with `finalOutcome: 'victory'`; `buildCombatSummary`
   reports the outcome and the store grants full XP + the weighted
   loot roll. The `CombatOutcome` union is
   `'victory' | 'mercy' | 'defeat' | 'retreat'`.
4. **Mercy / friendship path** — a disarming hit that softens a
   low-HP foe opens a mercy choice; `selectMercyChoice(state, 'spare')`
   spares the foe and ends the encounter with `finalOutcome: 'mercy'`
   (grants half-XP + full loot + a +1 moralMeter shift), while
   `'exploit'` trades the opening for a heavy finishing strike.
...
6. **Per-enemy befriend predicate** (Phase 68 / 112) — if the enemy
   carries `Enemy.befriendabilityConfig`, whether an explicit Befriend
   attempt succeeds is gated by an AND-composed predicate set
   (`roundsThreshold` / `hpGate { belowPct }` / `requiredStances[]` /
   `requiredSkillUse[]` / `defaultFallback`). `isBefriendAttemptEligible`
   is the single decision point the shared skill engine consults via
   `executeSkill`; the `requiredStances` / `requiredSkillUse` predicates
   derive from `state.log`. A successful Befriend opens the spare/exploit
   mercy choice above. First boss-tier authored config: `CoastalTyrant`
...
hazard / loot-cache / quest / narration — ten kinds today).
See [`world.md` § "Demo Content"](./world.md#demo-content-fishing-village)
for the full layout.

### Save / load

`store.save()` writes the current `GameState` (Phase 51 throttled to
durable actions only — `LEVEL_UP`, `END_COMBAT`, `MOVE_TO_NODE`,
`APPLY_DIALOGUE`, `SAVE_GAME`; Phase 72 added `RESET_RUN`; Phase 73
added `UNLOCK_CODEX_ENTRY`). `store.load()` restores via the configured
`PersistenceAdapter` + `migrate()` ladder (`GAME_STATE_VERSION = 10`;
`migrateV4toV5` defaults `philosophicalAlignment` to `{0,0,0}`;
`migrateV5toV6` defaults `runId` via `generateRunId(() => getRng().random())`;
`migrateV6toV7` defaults `codex` to `{ unlockedEntries: [] }`; steps
through `migrateV9toV10` complete the ladder).
...
| Combat | [quickstart-combat.md](./quickstart-combat.md) | `initializeCombatEncounter`, `playCombatCard`, `resolveThreatPhase`, event stream, mercy path |
