...
- [ ] **[E — docs gap] `chooseSubquest`, `generateSubquestDraft`, `selectSubquestFromDraft` absent from spec.md Hazard row and bearings.md Hazard section**
  - area: docs / Hazard engagement API
  - observation: The Hazard wildcard export (`export * from './World/Hazard'`) ships `chooseSubquest`, `generateSubquestDraft`, and `selectSubquestFromDraft` as key engagement-layer state-machine functions (`src/World/Hazard/hazard.engagement.ts`). All three are listed in `docs/hazard-minigame-api.md` with usage examples and recently gained hermetic e2e coverage (`src/World/Hazard/e2e/hazard.engagement.engine.test.ts`, commit `10a18f5`). The `bearings.md` Hazard section only explicitly calls out `dieCanPower`/`dieCanPowerCard` (combat-bridge predicates) and `confirmHazardForetell` (foretell state-machine entry) — the subquest engagement state-machine functions are absent. Similarly, the `spec.md` Hazard row notes `*` + "key helpers" but does not name these three. The pattern for `confirmHazardForetell` was explicitly added as a "Key state-machine entry point" in the bearings.md Hazard section; `selectSubquestFromDraft` / `chooseSubquest` / `generateSubquestDraft` deserve the same treatment.
  - suggested_fix: Add a "Key engagement-layer state-machine functions" paragraph to bearings.md Hazard section documenting `generateSubquestDraft(subquests, rng, count)` (generates the draft subquest candidates from the pool), `chooseSubquest(draft, id)` (picks one candidate from the draft, returning an updated HazardSubquestDraft), and `selectSubquestFromDraft(session, id)` (applies the chosen subquest to a live HazardSessionState). Extend the spec.md Hazard row to name these three alongside confirmHazardForetell.
  - score: 3 × 9 / 10 = 2.7
  - source: audit pass 116 (2026-06-26)
