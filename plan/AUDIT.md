...
- [ ] **[E — docs gap] `hazardCardPowerColors` absent from spec.md Contracts Hazard row and bearings.md Hazard block** — `hazardCardPowerColors(def)` is in the public fixture (via `export * from './World/Hazard'`) and documented in `docs/hazard-minigame-api.md` (line 56 import, line 180 usage note), but absent from the spec.md Contracts Hazard row and bearings.md Hazard annotation, which only call out `dieCanPower`/`dieCanPowerCard` as the "key combat-bridge predicates". `hazardCardPowerColors` is a sibling predicate (returns the set of die colors that can power a card) just tested at `37a928a` — same accumulation pattern as passes 88/89 (`SYNTHETIC_CARD_IDS`/`dieCanPower`/`dieCanPowerCard` doc gap, drained at `b807c46`). Score: 3 × 9 / 10 = 2.7.
