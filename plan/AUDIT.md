...
- **[Z-LOW] api — spec.md Contracts section missing recent exports** (score: 2.7). Two exports present in src/index.ts are not documented in spec.md Contracts section: enemyStatBudget (Enemy group) and getEffectsResolutionOutcome (Combat group). Missing from spec.md line ~89 table. Impact 3 (external consumers use spec.md as API reference), ease 9 (simple table addition). Source: critique-58 (commit 5f68b7f).
