...
- [ ] **[A — test coverage gap] `handCards` and `resolveThreatPhase` have zero direct hermetic test cases** — both are public barrel exports used by mobile to render the combat UI (`handCards` returns the player's current hand; `resolveThreatPhase` is the enemy-turn resolver that fires the telegraphed threat and advances to between-phases). Neither is directly imported or called in any `*.engine.test.ts` file (only exercised indirectly via the sim). Score: 4 × 8 / 10 = 3.2. Suggested fix: add 3–4 dedicated cases to `hazard-pattern-combat-helpers.engine.test.ts` — handCards returns cards from state.hand (empty before deal, populated after rollEncounterDice); resolveThreatPhase fires the threat action and advances phase (mark=overwhelmed when enemy acts, mark=clear when hindered). Source: iterate audit pass 117 (2026-06-26).
