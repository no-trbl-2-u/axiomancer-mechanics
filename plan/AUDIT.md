...
-->
<!-- iterate audit pass 123 (2026-06-27 at commit 1e7046d): 1 finding, top score 2.7. 4 commits since expand pass 86 at dd11ed3. Categories Z–H walked: **Z (external critique)** — all `[x]` (critique-95 LOW fully drained). **A (tests)** — all 147 `*.engine.test.ts` files present; no raw `vi.spyOn(Math,'random')` outside test-utils; 2111 tests passing. **B (spec gaps)** — only Q28 deferred (endgame); stale Specs 26-30 await /oversight. **C (type safety)** — zero `@ts-ignore`/`as any` in non-test src; lint exits 0. **D (dead code)** — none found. **E (docs)** — **[top finding]** `docs/combat.md` Combat Mechanics API table missing entries for ~13 locked-contract exports added to spec.md at `4890782`: `isAlive`, `isDefeated`, `getHealthPercentage`, `updateEffectDuration`, `getActiveEffectModifiers`, `getEffectiveStats`, `calculateEnemyStatMultiplier`, `applyMoralMeterScaling` (functions) + `Advantage`, `CritStyle`, `CombatAction`, `CombatPhase`, `AggregatedEffectModifiers`, `EffectiveStats`, `DamageType` (types). Score 3 × 9 / 10 = 2.7. **F (ESLint)** — clean. **G (deps)** — `@types/node` 25→26 major bump only (skip per policy). **H (hygiene)** — working tree clean. Top finding: [E] docs/combat.md missing ~13 foundational Combat API entries (score 2.7). -->
<!-- iterate audit pass 121 (2026-06-27 at commit dd11ed3): 0 findings. 15 commits since expand pass 85 at 6feb24a — the window is entirely iterate/docs work: Phase 166 terminate findings (CombatCard.skillId docs at 8427eb6, CHANGELOG at b93187d, bottomDamagePreview sub-barrel at 39eb361, CombatAttributionRow/LandedEffect docs at 9f46098, buildCombatSummary attribution depth at 58ca7bd). Categories Z–H walked: **Z (external critique)** — all `[x]` (critique-95 LOW fully drained at 39eb361). **A (tests)** — all 147 `*.engine.test.ts` files present; no raw `vi.spyOn(Math,'random')` outside test-utils; attribution coverage extended at 58ca7bd. **B (spec gaps)** — only Q28 deferred (endgame content-authoring, no engine work planned); stale Specs 26-30 await /oversight. **C (type safety)** — zero `@ts-ignore`/`as any` in non-test src; `npm run lint` clean. **D (dead code)** — none found; `drawCombatCards`/`shuffleCombatDeck` in Combat sub-barrel are internal cross-file imports, not public-API leaks (not on root barrel). **E (docs)** — `isAlive`/`getHealthPercentage` (barrel since early module split) absent from spec.md Combat row and docs/combat.md API table — score 2 × 9 / 10 = 1.8 (below 3.0 threshold; old foundational exports, not fresh accumulation). **F (ESLint)** — clean. **G (deps)** — `@types/node` 25→26 major bump only (T approval required; skip). **H (hygiene)** — plan files current. No finding scores ≥3.0; dispatching to /expand per failure mode 3 (bold posture). Pool: 0 new findings. -->
...
- [x] **[E — docs gap] `docs/combat.md` Combat Mechanics API table missing ~13 locked-contract exports added to spec.md at `4890782`** — resolved at `ee4446f` (iterate, 2026-06-27). Added `isAlive`, `isDefeated`, `getHealthPercentage`, `updateEffectDuration`, `getActiveEffectModifiers`, `getEffectiveStats`, `calculateEnemyStatMultiplier`, `applyMoralMeterScaling` to the Combat Mechanics API table; added a new Combat Types sub-table covering `Advantage`, `CritStyle`, `CombatAction`, `CombatPhase`, `AggregatedEffectModifiers`, `EffectiveStats`, `DamageType`. Doc-only; 2111 tests + type-check + lint + build green. Score: 3 × 9 / 10 = 2.7. Source: iterate audit pass 123 (2026-06-27).
- [x] **[E — docs gap] spec.md Contracts Combat row missing ~30 foundational locked-contract exports** — resolved at `4890782` (iterate, 2026-06-27). The Combat row opened with `determineAdvantage, getAttackStat, getDefenseStat, applyDamage, heal, Stance, Action, CombatState` then jumped to the full Spec 25 surface, silently omitting ~30 locked-contract exports canonical per bearings.md. Added `getBaseStat`, `getSaveStat`, `isAlive`, `isDefeated`, `getHealthPercentage`, `getStudyMarkIntensity`, `getActiveRollModifier`, `getThornsReflect`, `updateEffectDuration`, `tickAllEffects`, `removeRandomBuff`, `extendRandomBuffDuration`, `applyRegen`, `getActiveEffectModifiers`, `getEffectiveStats`, `canAct`, `resolveEffectApplication`, `calculateDamageResistance`, `getSkillDamageType`, `determineEnemyAction`, `isCombatOngoing`, `determineCombatEnd`, `healCharacter`, `calculateEnemyStatMultiplier`, `applyMoralMeterScaling` (functions) + `Combatant`, `Advantage`, `CritStyle`, `CombatAction`, `CombatPhase`, `BattleLogEntry`, `AggregatedEffectModifiers`, `EffectiveStats`, `DamageType` (types) to the Contracts Combat row. Doc-only; type-check clean. Score: 4 × 9 / 10 = 3.6. Source: iterate audit pass 122 (2026-06-27).
...
- [x] **[A — test coverage gap] `handCards` and `resolveThreatPhase` have zero direct hermetic test cases** — resolved at `7948b77` (iterate, 2026-06-26). Added 6 new hermetic cases across 2 describe blocks in `hazard-pattern-combat-helpers.engine.test.ts`: handCards (3 cases — opening-hand uid+card pairs, order preservation, empty-hand guard); resolveThreatPhase (3 cases — mark=overwhelmed when enemy acts, mark=clear when hindered by sleep/skipTurn effect, no-op when combat complete). 2095 tests + type-check + lint + build green. Score: 4 × 8 / 10 = 3.2. Source: iterate audit pass 117 (2026-06-26).
...
- [x] **[Z — critique-90 LOW-1] `CardDieCost` not listed as named type export in spec.md + bearings.md types sequence** — resolved at `cc93eb0` (iterate, 2026-06-25). Added `CardDieCost` as a named type to spec.md die-cost annotation and bearings.md die-cost block. Score 2 × 9 / 10 = 1.8. Source: critique-90 (commit 6569da3).
...
- [x] **[E — docs drift] Phase 142 public exports absent from docs/effects.md, docs/skills.md, spec.md, and bearings.md** — resolved at `368d542` (iterate, 2026-06-25). Added §Phase 142 — Status Effect Interaction Engine table to docs/effects.md (14 entries: EFFECT_INTERACTIONS, evaluateInteractions, checkInteractionTrigger, applyInteractionResult, getInteractionsForEffect, getAllInteractionIds, getInteractionById, validateInteractions, INTERACTION_AMPLIFICATION, INTERACTION_PRIORITY, STATUS_RESOLUTION_DEBUFF_THRESHOLD/DOT_THRESHOLD/DOT_MAX_ROUNDS/ENGAGEMENT_FLOOR_PERCENT); added §Phase 142 — Extended Synergy Predicates table to docs/skills.md (7 entries: evaluateExtendedSynergyPredicate + 5 check helpers + ExtendedSynergyPredicate); updated spec.md Contracts Effects + Skills rows; updated bearings.md Effects + Skills groups. Doc-only; 2041 tests + type-check + lint + build green. Score 3 × 8 / 10 = 2.4.

- [x] **[E — docs drift] `STARTING_SKILL_IDS` absent from `docs/combat.md` deckbuilder API row** — resolved at `d996fb1` (iterate, 2026-06-25). `STARTING_SKILL_IDS` (preferred two-element array `['slippery-slope', 'brace-for-impact']` that seeds `knownSkills` with both offense + GUARD defense) is a public root barrel export documented in spec.md and bearings.md, but the `docs/combat.md` deckbuilder table row only listed its scalar sibling `STARTING_SKILL_ID`. Extended the row to include `STARTING_SKILL_IDS` with a description of the preference distinction. Doc-only; type-check + lint green. Score 2 × 9 / 10 = 1.8.
...
- [x] **[E — docs drift] `confirmHazardForetell` and MTG-expansion keyword fields absent from `docs/hazard-minigame-api.md`** — resolved at `ee5775e` (iterate, 2026-06-26). Added `confirmHazardForetell` to the Package surface import list; updated state machine to show `foretell-pending` branch; extended host flow example with the foretell-pending guard + confirmHazardForetell call; added MTG-inspired card expansion section documenting `foretell`/`echo`/`scour` keywords and all 8 new `HazardCardDef` fields. Doc-only; type-check + lint green. Score: 4 × 9 / 10 = 3.6.
...
- [x] **[A — test coverage gap] `chooseSubquest` has zero dedicated hermetic tests** — resolved at `10a18f5` (iterate, 2026-06-26). Imported `chooseSubquest` in `hazard.engagement.engine.test.ts`; added 3 dedicated cases under Sub-quest Drafting (stamps chosen by id; preserves candidates list; unknown id sets chosen to null) and 1 acceptance-criteria case. 2059 tests green (+3). Score 4 × 8 / 10 = 3.2.

- [x] **[E — docs gap] `chooseSubquest`, `generateSubquestDraft`, `selectSubquestFromDraft` absent from spec.md Hazard row and bearings.md Hazard section** — resolved at `a0edfbb` (iterate, 2026-06-26). Added "Key engagement-layer state-machine functions" paragraph to bearings.md Hazard block documenting all three; extended spec.md Hazard row to name `generateSubquestDraft`/`chooseSubquest`/`selectSubquestFromDraft` alongside `confirmHazardForetell`. Doc-only; type-check + lint green. Score 3 × 9 / 10 = 2.7. Source: audit pass 116 (2026-06-26).
- [x] **[E — docs gap] JEOPARDY/MIRACLE/BUYBACK/DELVE mechanics absent from `docs/hazard-minigame-api.md` MTG card expansion table (2026-06-26 heavy-card-library-swap)** — resolved at `bc230bd` (iterate, 2026-06-26). Extended the MTG-inspired card expansion table header to note v0.32.2 additions; added 8 new rows covering `effect:'jeopardy'`/`jeopardyForce`/`jeopardyEscape`, `effect:'miracle'`/`firstPlayForce`/`firstPlayEscape`, `buyback` (boolean), `effect:'delve'`/`delveForce`/`delveEscape` — each with trigger condition, engine behaviour, and mobile host-side interruption note (none required for the 4 new mechanics). Doc-only; type-check + lint green. Score 4 × 9 / 10 = 3.6. Source: iterate audit 2026-06-26.

- [x] **[E — docs gap] `docs/equipment.md` affix draw weight description omits `STATUS_AFFIX_DRAW_BIAS` multiplier (Phase 157)** — resolved at `b7d38da` (iterate, 2026-06-26). Extended the affix draw weight paragraph to document `isOffensiveStatusAffix` classification, `STATUS_AFFIX_DRAW_BIAS` (default 3), the effective weights (30/9/3 for status vs 10/3/1 flat-stat), and the tunable registry key `loot.statusAffixDrawBias`. Doc-only; type-check + lint + build green. Score 3 × 9 / 10 = 2.7. Source: iterate audit 2026-06-26.

- [x] **[A — test coverage gap] `dieCanPower` / `dieCanPowerCard` / `hazardCardPowerColors` have zero dedicated hermetic test cases** — resolved at `37a928a` (iterate, 2026-06-26). Added 16 hermetic cases across 3 describe blocks in `hazard.engine.test.ts`: dieCanPower (4 cases — matching color, gold wild, mismatch, hex block); dieCanPowerCard (9 cases — single-color match/mismatch/gold/hex + two-tone first-color, second-color, unrelated, gold, hex); hazardCardPowerColors (3 cases — single-color kind fallback, red/blue two-tone, red/purple two-tone). 2089 tests + type-check + lint + build green. Score 4 × 8 / 10 = 3.2. Source: iterate audit 2026-06-26.
...
- [x] **[Z — critique-83 LOW-1] docs/world.md narration MapEventKind missing from event-kind table** — resolved at `bfb0b73` (iterate, 2026-06-23). `docs/world.md` prose updated from "nine" to "ten" MapEventKind values, and a `narration` row added to the discriminated-union table with `{ kind: 'narration', dialogue }` shape + `NarrationPayload` barrel reference. "All 8 MapEventKind" count in the fishing-village content section and the content-parity guard note intentionally unchanged (narration not required in map pools). Doc-only; 2004 tests + type-check + lint + build + deploy:check green. Score 3 × 9 / 10 = 2.7. Source: critique-83 (commit bf56ab8). CRITIQUE.md LOW-1 moved to Done.

- [x] **[Z — critique-83] spec.md contracts table stale Combat row post HP-only model** — resolved at `65c7d4e` (iterate, 2026-06-23). Removed `CombatPressureTracks` (barrel-removed at cb67ad3), replaced `projectCardPressure` with `projectCardImpact`, added HP-only model note to Spec 25 annotation, added defense cards / GUARD mechanic with `GOLD_CARD_IDS`/`isGoldCard`/`CardEffectKind`, added `STARTING_SKILL_IDS` to deckbuilder annotation, added `NarrationPayload` to World row. Doc-only; 2004 tests + type-check + lint + build + deploy:check green. Score 5 × 7 / 10 = 3.5. Source: critique-83 (commit bf56ab8). CRITIQUE.md MED-2 moved to Done.
...
- [x] **[H — release bookkeeping] CHANGELOG/tag drift breaks `npm run deploy:check`** — resolved at `85c80f4` (2026-06-22). The latest git tag was `v0.29.0` and `package.json` `0.29.0`, but the top *tagged* CHANGELOG heading was `[0.22.0]` (the `[0.23.0]` block was still marked `— unreleased`). The Phase 52 deploy-check guard (`scripts/deploy-check.mjs:107-121`) requires the normalised latest tag to equal the top non-unreleased CHANGELOG version, so `npm run deploy:check` exited 1 on a "git tag / CHANGELOG.md disagreement" — the publish/release gate was red while type-check / lint / build / public-surface snapshot were all green. Six release commits shipped after the `[0.23.0] — unreleased` block was opened (2026-06-20) without ever dating it. Fix: dated the `[0.23.0]` block (2026-06-20) and added a dated consolidated `[0.29.0] — 2026-06-22` heading covering the post-0.23 work (Spec 25 Hazard-Pattern Combat, Spec 26/26b combat redesign, the 0.24.x additive surface, Rest/LootCache sims), so the top tagged heading matches `v0.29.0`. deploy:check now exits 0 ("Package is publishable"). Doc-only; verify gates green. Score 7 × 8 / 10 = 5.6.

- [x] **[E — docs drift] Spec 26/26b combat surface absent from every front-door reader** — resolved at `5e8385c` (2026-06-22). PR #184 (`6378e92`) landed the in-flight Spec 26/26b combat redesign (stance draft, hidden-stance read, Conviction economy, Signature Skills, deckbuilder card rewards, player archetypes) adding ~40 value/type exports to the locked root barrel via `src/Combat/index.ts`: the turn lifecycle (`startTurn`/`draftStanceDie`/`endTurn`/`resolveRead`/`chooseDraft`/`discardCombatCard`/`getDraftedDie`/`isPhaseStanceRevealed`/`revealedCurrentStance`/`cardReadPreview`/`projectCardPressure`), the Conviction + read tuning constants (`READ_PRESSURE_MULT`/`CONVICTION_PER_UNPICKED_DIE`/`CONVICTION_READ_WIN_BONUS`/`COLOR_MATCH_PRESSURE_BONUS`/`TURN_DICE_COUNT`/`rollTurnDice`/`dieHasStance`/`deriveIntentType`), Signature Skills (`playSignatureSkill`/`getSignatureSkill`/`SIGNATURE_SKILLS`/`SIGNATURE_SKILL_LIST`/`SIGNATURE_KITS`/`signaturesForArchetype`/`playerArchetype`), and the deckbuilder rewards (`COMBAT_REWARD_POOL`/`STARTING_SKILL_ID`/`rollCombatCardRewards`/`addRewardCard`/`unlockSkillViaDilemma`) + types (`CombatIntentType`/`CombatReadResult`/`SignatureSkill`/`SignatureSkillId`/`SignatureSkillKind`/`PlayerArchetype`). They reached the public-surface fixture (deploy:check "publishable") and carry e2e coverage (30 refs in `hazard-pattern-combat.engine.test.ts`), but touched NO front-door reader: `docs/combat.md` §Hazard-Pattern Combat documented only the Spec 25 surface, `spec.md` Contracts Combat row listed only the Spec 25 marquee, and `plan/bearings.md` Combat group had zero Spec 26/26b mention (grep = 0 across all three). Same front-door-doc-lag shape as the Spec 25 landing (`cc0aa0a`) and Phase 152/154 findings. Added: a `### Spec 26 / 26b` subsection + API table to `docs/combat.md` (with the "no spec file yet — distinct from specs/26-catalyst" caveat), the Spec 26/26b marquee on the `spec.md` Contracts Combat row, and a Spec 26/26b annotation block on the `bearings.md` Combat group. All ~40 documented symbols verified as real `src/index.ts` exports (value + type greps). Doc-only; type-check + lint + build + deploy:check green. Score 5 × 7 / 10 = 3.5.

- [x] **[A — test-coverage gap] 14 Spec 25 public exports have no direct e2e coverage** — resolved at `1addaab` (2026-06-21). The Spec 25 ship (`a2112ba`) added one hermetic flow suite (`src/Combat/e2e/hazard-pattern-combat.engine.test.ts`, the §11 acceptance criteria) that exercised the engine end-to-end but referenced only ~20 of the ~34 newly-exported value functions by name. Cross-checking every Spec 25 barrel value against the suite surfaced 14 exports with zero direct test reference: the two **win-condition predicates** `dotErosionReached` / `controlSaturationReached` (the Pressure-Track thresholds that ARE the win condition per the load-bearing doctrine), the Befriend mercy entry `selectEncounterMercyChoice`, the dice helpers `rollCombatDice` / `combatDieCanPower` / `refreshOneDie` (the self-reinforcing status-loop primitive), `momentumCarry`, `buildCombatDeck`, `generateDefaultThreatSequence`, card adapters `classifyVerbClass` / `toCombatCard` / `projectDeck`, and UI previews `cardDieCostPreview` / `availableDice`. Added `src/Combat/e2e/hazard-pattern-combat-helpers.engine.test.ts` — a focused hermetic sibling suite (pure math + explicit fixed RNG, no disk/network/TTY) giving each of the 14 a direct contract test. All 22 assertions validated against the built engine via a throwaway ts-node harness (vitest itself is blocked in-container by the known ERR_REQUIRE_ESM vite/vitest mismatch); type-check + type-check:tests + lint + build green. No source change. Score 6 × 8 / 10 = 4.8.
...
### 2026-06-27 (Pass 119) — Z–H walk (march→iterate dispatch)
Z: CRITIQUE.md Pending all `[x]` (fully drained — critique-95 LOW resolved at `39eb361`). A: 147 test files / 2111 tests passing; `vi.spyOn(Math,'random')` clean outside test-utils; no new `*.engine.test.ts` gaps — Phase 166 added `terminology-boundary.engine.test.ts` (regression guard only). B: Specs 26-30 blank `> Your answer:` stale (Pressure-Track model removed; await `/oversight`); Knowledge-Gaps Q28 deferred (endgame). C: zero `@ts-ignore`/`as any` in non-test src; lint exits 0. D: no dead code (public-surface fixture clean). E: **[top finding]** CHANGELOG `[Unreleased]` section missing Phase 166 terminology boundary entry — Phase 166 shipped terminology cleanup, regression guard, and docs but touched no CHANGELOG. Score 2 × 10 / 10 = 2.0. F: lint green. G: `@types/node` 25→26 major (skip per policy). H: working tree clean. Top finding: [E] Phase 166 CHANGELOG entry missing (score 2.0). Shipped at `b93187d`.

### 2026-06-26 (Pass 117) — Z–H walk (march→iterate dispatch)
Z: CRITIQUE.md Pending all `[x]` (fully drained — critique-94 LOW resolved at `7a35a90` this window). A: **[top finding]** `handCards` and `resolveThreatPhase` are public barrel exports used by mobile but have zero direct hermetic test cases in any `*.engine.test.ts`; both are only exercised indirectly via the sim. Score 4 × 8 / 10 = 3.2. B: Specs 26-30 blank `> Your answer:` stale (Pressure-Track model removed; await `/oversight`); Knowledge-Gaps Q28 deferred (endgame). C: zero `@ts-ignore`/`as any` in non-test src; lint exits 0. D: no dead code. E: docs current (pass-94 LOW fully drained). F: lint green. G: `@types/node` 25→26 major (skip per policy). H: working tree clean (4 commits since critique pass 94 at 2347d8a). Top finding: [A] `handCards`/`resolveThreatPhase` missing direct hermetic cases (score 3.2). Shipping this tick.

### 2026-06-26 (Pass 116) — Z–H walk (march→iterate dispatch)
Z: CRITIQUE.md Pending all `[x]` (fully drained — critique-92 findings resolved at 5980a49/1ba3cce this window). A: all 146 test files passing (2059 tests); `vi.spyOn(Math,'random')` clean outside test-utils; no new `*.engine.test.ts` gaps — `hazard.engagement.engine.test.ts` recently gained chooseSubquest coverage (10a18f5). B: Specs 26-30 blank `> Your answer:` stale (Pressure-Track model removed; await `/oversight`); Knowledge-Gaps Q28 deferred (endgame). C: zero `@ts-ignore`/`as any` in non-test src; lint exits 0. D: no dead code. E: **[top finding]** `chooseSubquest`/`generateSubquestDraft`/`selectSubquestFromDraft` are key Hazard engagement state-machine functions in the wildcard barrel export and documented in `docs/hazard-minigame-api.md`, but absent from `bearings.md` Hazard section and `spec.md` Hazard row; consistent with the `confirmHazardForetell` addition pattern. Score 3 × 9 / 10 = 2.7. F: lint green. G: `@types/node` 25→26 major (skip per policy); no other outdated packages. H: working tree clean (11 commits since critique pass at 3dc07bb; 26 commits since expand pass at 1adc128). Top finding: [E] engagement-layer state-machine functions absent from bearings.md + spec.md (score 2.7). Shipping this tick.
### 2026-06-25 (Pass 115) — Z–H walk (march→iterate dispatch)

Z: CRITIQUE.md Pending all `[x]` (fully drained — critique-90 LOWs fully resolved across this window at d996fb1/783ca9e/cc93eb0/e2eca58). A: all 126 engine test files passing; `vi.spyOn(Math,'random')` clean outside test-utils; 146 test files / 2041 passing. B: Specs 26-30 blank `> Your answer:` stale (Pressure-Track model removed; await `/oversight`); Knowledge-Gaps Q28 deferred (endgame). C: zero `@ts-ignore`/`as any` in non-test src; lint exits 0. D: no dead code (surface byte-identical to expected.json, 585 values / 396 types). E: **[top finding]** `STARTING_SKILL_IDS` (public barrel export, documented in spec.md/bearings.md) absent from `docs/combat.md` deckbuilder table row — only its scalar sibling `STARTING_SKILL_ID` was listed. Score 2 × 9 / 10 = 1.8. Also noted: `PASSIVE_DEFENSE_MULTIPLIER` not in docs/game.md or spec.md/bearings.md Game section (only appears in README.md prose; score 1.8 — next iterate queue). F: lint green. G: `@types/node` 25→26 major (skip per policy). H: working tree clean. Top finding: [E] `STARTING_SKILL_IDS` absent from `docs/combat.md` (score 1.8). Shipped at `d996fb1`.

### 2026-06-25 (Pass 114) — Z–H walk (march→iterate dispatch)

Z: CRITIQUE.md Pending has 1 LOW from critique-88 — `resolveCardDieCost`/`cardDieCostPreview` absent from spec.md/bearings.md. A: all modules have `e2e/*.engine.test.ts`; `vi.spyOn(Math,'random')` clean outside test-utils; 146 test files / 2041 passing. B: Specs 26-30 blank `> Your answer:` stale (Pressure-Track model removed; await `/oversight`); Knowledge-Gaps Q28 deferred (endgame). C: zero `@ts-ignore`/`as any` in non-test src; lint exits 0. D: no dead code. E: docs current post-fix. F: lint green. G: `@types/node` 25→26 major (skip per policy). H: working tree clean. Top finding: [Z] critique-88 LOW-2 — `resolveCardDieCost`/`cardDieCostPreview` absent from spec.md Contracts Combat row and bearings.md (score 3 × 9 / 10 = 2.7). Shipped at d076a0d. Queue now empty (only `[needs-user-call — DIV-MECH-005]` outstanding, awaiting T decision).
...
**Findings:** 0 findings ≥3.0 — queue empty. HEAD = `30204f5` (the pass-106 audit-bookkeeping commit itself); **zero `src/**` delta since pass 106** (`git diff --stat 7a4a773..HEAD -- src/` empty — the tree audited here is byte-identical to the tree pass 106 audited). Re-walked every category independently against the live tree; all four reliable gates exit 0: `type-check`, `lint`, `build`, `deploy:check` ("Package is publishable"), and the public surface is **byte-identical** to `scripts/public-surface.expected.json` (live snapshot diff empty).

- **Z (Critique):** CRITIQUE.md `## Pending` empty (critique pass 81 at `40d4acd`, 0 findings — "Spec 25 combat landing clean"). Critique gate not re-due (6 commits since `40d4acd` < 12; ~6h since 2026-06-21 20:17 UTC < 24h).
- **A (Tests):** every src module retains an `e2e/` dir; the Spec 25 engine carries the acceptance suite (`hazard-pattern-combat.engine.test.ts`), the per-export helper suite (`hazard-pattern-combat-helpers.engine.test.ts`), and the balance sim guard (`hazard-pattern-combat.balance.sim.test.ts`). Independently re-grepped: zero `vi.spyOn(Math,'random')` outside `src/test-utils/rng.ts`. (`npm test` red on the known environmental ERR_REQUIRE_ESM vite/vitest mismatch; the four exit-0 gates + byte-identical surface are the reliable witnesses.)
- **B (Spec-gaps):** Knowledge-Gaps all resolved save Q28 (deferred — endgame, content-authoring); Specs 26-30 (catalyst scaling / card salvage / curated deck synergy / reactive enemies / lethality readout) remain forward-looking proposals filed by expand pass 75, **correctly unimplemented** — they await `/oversight` promotion, not iterate drains.
- **C (Type-safety):** clean — independent grep finds zero `as any`/`as unknown`/`@ts-ignore`/`@ts-expect-error` in non-test `src/`.
...
- **E (Docs):** front-door readers current — `docs/combat.md` §Hazard-Pattern Combat + spec.md Contracts Combat row + bearings.md Spec 25 clause all landed (cc0aa0a); `docs/enemy.md` "63 authored enemies" verified accurate against the live `dist` registry (63); no `src/` delta since to drift against.
- **F (Lint):** `npm run lint` exits 0. **G (Deps):** only `@types/node` 25→26 major outstanding (Current==Wanted 25.9.4; deliberate-phase territory, skip per category G). **H (hygiene):** working tree clean at audit start.

**Disposition:** no drainable finding. Bold posture would dispatch to `/expand` per iterate failure-mode 3, BUT the expand signal gate is not met — AUDIT Pending empty, CRITIQUE Pending empty, Knowledge-Gaps exhausted (only Q28 deferred), all braindump entries implemented; the only fresh signal cluster (hazard-pattern-combat open questions → Specs 26-30) was **already filed by expand pass 75** and awaits `/oversight`, AND the expand rate-limit gate itself is not met (4 commits since pass 75 < 20; ~5h < 48h). Re-running expand would re-walk an identical signal set with no new commits → 0 new candidates. Return cleanly; next loop tick re-dispatches (critique becomes eligible at ≥12 commits or >24h from `40d4acd`).

### 2026-06-22 (Pass 106) — Z–H walk (march→iterate dispatch)

**Categories audited:** External critique (Z), Test-quality gaps (A), Spec-gap items (B), Type-safety (C), Dead code (D), Documentation gaps (E), ESLint fix (F), Dependency updates (G), Commit-hygiene (H).

**Findings:** 0 findings ≥3.0 — queue empty. HEAD = `7a4a773` (the pass-105 audit-bookkeeping commit itself); **zero `src/**` delta since pass 105** (`git diff --stat HEAD~5..HEAD -- src/` empty — the whole window since the Spec 25 cluster is bookkeeping: expand pass 75 candidate filing + audit passes 103-105). Re-walked every category independently against the live tree; all four reliable gates exit 0: `type-check`, `lint`, `build`, `deploy:check` ("Package is publishable").

- **Z (Critique):** CRITIQUE.md `## Pending` empty (critique pass 81 at `40d4acd`, 0 findings — "Spec 25 combat landing clean"). Critique gate not re-due (5 commits since `40d4acd` < 12; ~6h < 24h).
- **A (Tests):** every src module retains an `e2e/` dir; the Spec 25 engine carries the acceptance suite (`hazard-pattern-combat.engine.test.ts`), the per-export helper suite (`hazard-pattern-combat-helpers.engine.test.ts`), and the balance sim guard (`hazard-pattern-combat.balance.sim.test.ts`). Independently re-grepped: zero `vi.spyOn(Math,'random')` outside `src/test-utils/rng.ts`. (`npm test` red on the known environmental ERR_REQUIRE_ESM vite/vitest mismatch; the four exit-0 gates are the reliable witnesses.)
- **B (Spec-gaps):** Knowledge-Gaps all resolved save Q28 (deferred — endgame, content-authoring). The 20 `specs/**` files carrying `Your answer:` markers are either the implemented specs (01-14, answers filled) or the Specs 26-30 forward-looking depth proposals (catalyst scaling / card salvage / curated deck synergy / reactive enemies / lethality readout) filed by expand pass 75 — **correctly unimplemented**, awaiting `/oversight` promotion, not iterate drains.
- **C (Type-safety):** clean — independent grep finds zero `as any`/`as unknown`/`@ts-ignore`/`@ts-expect-error` in non-test `src/`.
- **D (Dead code):** independent grep finds zero `TODO`/`FIXME`/`HACK`/`XXX` markers in non-test `src/`; the Spec 25 engine ships additively alongside `resolveCombatRound`; all barrel exports reachable; public surface byte-identical to fixture (deploy:check "publishable").
- **E (Docs):** front-door readers current — `docs/combat.md` §Hazard-Pattern Combat + spec.md Contracts Combat row + bearings.md Spec 25 clause all landed (cc0aa0a); `docs/enemy.md` "63 authored enemies" verified accurate; no `src/` delta since to drift against.
- **F (Lint):** `npm run lint` exits 0. **G (Deps):** only `@types/node` 25→26 major outstanding (deliberate-phase territory, skip per category G). **H (hygiene):** working tree clean at audit start.

**Disposition:** no drainable finding. Bold posture would dispatch to `/expand` per iterate failure-mode 3, BUT the expand signal gate is not met — AUDIT Pending empty, CRITIQUE Pending empty, Knowledge-Gaps exhausted (only Q28 deferred), all braindump entries implemented; the only fresh signal cluster (hazard-pattern-combat open questions → Specs 26-30) was **already filed by expand pass 75** and awaits `/oversight`, AND the expand rate-limit gate itself is not met (3 commits since pass 75 < 20; ~5h < 48h). Re-running expand would re-walk an identical signal set with no new commits → 0 new candidates. Return cleanly; next loop tick re-dispatches (critique becomes eligible at ≥12 commits or >24h from `40d4acd`).

### 2026-06-22 (Pass 105) — Z–H walk (march→iterate dispatch)

**Categories audited:** External critique (Z), Test-quality gaps (A), Spec-gap items (B), Type-safety (C), Dead code (D), Documentation gaps (E), ESLint fix (F), Dependency updates (G), Commit-hygiene (H).

**Findings:** 0 findings ≥3.0 — queue empty. HEAD = `8cfe54c` (the pass-104 audit-bookkeeping commit itself); **zero `src/**` delta since pass 104** — the tree audited here is byte-identical to the tree pass 104 audited. Re-walked every category independently against the live tree; all reliable gates exit 0: `type-check`, `lint`, `build`, `deploy:check` ("Package is publishable"), and the public surface is **byte-identical** to `scripts/public-surface.expected.json` (live snapshot diff empty).

- **Z (Critique):** CRITIQUE.md `## Pending` empty (critique pass 81 at `40d4acd`, 0 findings — "Spec 25 combat landing clean"). Critique gate not re-due (4 commits since `40d4acd` < 12; ~9h < 24h).
- **A (Tests):** every src module retains an `e2e/` dir (loop confirmed: zero public modules without `e2e/`); the Spec 25 engine carries the acceptance suite + per-export helper suite + balance sim guard. Zero `vi.spyOn(Math,'random')` outside `src/test-utils/rng.ts`. (`npm test` red on the known environmental ERR_REQUIRE_ESM vite/vitest mismatch; the four exit-0 gates + byte-identical surface are the reliable witnesses.)
- **B (Spec-gaps):** Knowledge-Gaps all resolved save Q28 (deferred — endgame, content-authoring); Specs 26-30 (catalyst scaling / card salvage / curated deck synergy / reactive enemies / lethality readout) remain forward-looking proposals filed by expand pass 75, **correctly unimplemented** — they await `/oversight` promotion, not iterate drains.
- **C (Type-safety):** clean — zero `as any`/`as unknown`/`@ts-ignore`/`@ts-expect-error` in non-test `src/`.
- **D (Dead code):** the Spec 25 engine ships additively alongside `resolveCombatRound`; all barrel exports reachable; public surface byte-identical to fixture.
- **E (Docs):** front-door readers current — `docs/combat.md` §Hazard-Pattern Combat + spec.md Contracts Combat row + bearings.md Spec 25 clause all landed (cc0aa0a); no `src/` delta since to drift against.
- **F (Lint):** `npm run lint` exits 0. **G (Deps):** only `@types/node` 25→26 major outstanding (Current==Wanted; deliberate-phase territory, skip per category G). **H (hygiene):** working tree clean at audit start.

**Disposition:** no drainable finding. Bold posture would dispatch to `/expand` per iterate failure-mode 3, BUT the expand signal gate is not met — AUDIT Pending empty, CRITIQUE Pending empty, Knowledge-Gaps exhausted (only Q28 deferred), all braindump entries implemented; the only fresh signal cluster (hazard-pattern-combat open questions → Specs 26-30) was **already filed by expand pass 75** and awaits `/oversight`, AND the expand rate-limit gate itself is not met (2 commits since pass 75 < 20; ~9h < 48h). Re-running expand would re-walk an identical signal set with no new commits → 0 new candidates. Return cleanly; next loop tick re-dispatches (critique becomes eligible at ≥12 commits or >24h from `40d4acd`).
...
**Disposition:** DRAINED at `1addaab` — added `src/Combat/e2e/hazard-pattern-combat-helpers.engine.test.ts`, a focused hermetic sibling suite pinning the 14 untested Spec 25 exports (validated 22/22 assertions against the built engine via a throwaway ts-node harness, since vitest itself is blocked in-container by the ERR_REQUIRE_ESM mismatch). Expand gate not re-eligible this tick (26 commits since expand pass 74 ≥20, BUT no actionable signal source: CRITIQUE empty, Knowledge-Gaps exhausted, braindump all-implemented). Next loop tick re-dispatches.
...
**Findings:** 0 findings ≥3.0 — queue empty. The Pass-94 README affix-row drain (`f178dee`) closed the last live front-door docs-drift reader; Passes 91–94 swept all four stale references (`getResistStat` ×2, `SkillPhaseEvent`/`ResourceEvent` umbrella, README affix row). No new finding surfaced.

**Highlights (independently re-verified this pass):**
- **Z (Critique):** CRITIQUE.md Pending empty; last critique pass 75 at `607f4e9` (today), 0 findings.
- **A (Tests):** `npm test` green — 127 test files / 1821 passing (1 intentional skip). Zero `vi.spyOn(Math,'random')` outside the sanctioned `src/test-utils/rng.ts`.
- **B (Spec-gaps):** Knowledge-Gaps only Q28 deferred (endgame); `> Your answer:` blanks are template/instruction files only.
- **C (Type-safety):** clean — zero `as any`/`as unknown`/`@ts-ignore`/`@ts-expect-error` over non-test src.
- **D (Dead code):** no `TODO`/`FIXME`/`HACK`/`XXX` markers in non-test src.
- **E (Docs):** doc-count claims re-verified accurate against a fresh `npm run build` dist — ENEMY_REGISTRY = 63 = docs/enemy.md:86 "63 authored enemies"; equipmentTemplates = 56 = docs/equipment.md "56 entries"; prefixes = 36 / suffixes = 36 = docs/equipment.md affix tables. No stale removed-symbol references remain in front-door readers.
- **F (Lint):** build green. **G (Deps):** `npm outdated` clean (no minor/patch bumps). **H (hygiene):** `git status` clean at audit start; public-surface snapshot byte-identical to `scripts/public-surface.expected.json` (297 values / 202 types) after build.

**Disposition:** zero findings ≥3.0. Bold posture would normally dispatch to `/expand` (iterate failure mode 3), but the expand gate was already commit-distance-gated this tick (4 commits since expand pass 73 at `f44fde5`, need ≥20). Recording the audit and returning cleanly; next tick re-dispatches per march.
...
**Highlights (independently re-verified this pass):**
- **Z (Critique):** CRITIQUE.md Pending empty; last critique pass 74 at `5182884` (today), 0 findings. No new pass armed (3 commits < 12-commit cadence, <24h).
- **A (Tests):** `npm test` green — 127 test files / 1821 passing (1 intentional skip). `rg 'vi.spyOn\(Math'` over non-test-utils src = 0 hits (sole sanctioned hit is `src/test-utils/rng.ts`). No hermetic-coverage gap ≥3.0.
- **B (Spec-gaps):** Knowledge-Gaps only Q28 deferred (endgame, content-authoring); specs/README recommended-order has no stale NEXT markers; the 4 remaining `> Your answer:` blanks are all in `specs/**/00-*-template.md` author templates (intentionally blank), not live spec gaps.
- **C (Type-safety):** clean — `rg 'as any|@ts-ignore'` over non-test src = 0 hits; `npm run type-check` green (verify gate).
- **D (Dead code) / H (hygiene):** `git status` clean at audit start; the 3 commits since pass 89 are bookkeeping (audit pass 89 `05984b6`) + a critique pass-74 bookkeeping commit (`b81cd45`) + a CLI-only fix (`ddfc834` — `src/CLI/hazard.cli.ts`, UI-only, excluded from build/public API, no doc/contract drift).
- **E (Docs):** every `src/` module retains a `docs/` counterpart; contracts synced last window. The lone post-pass-89 source change is CLI-only — not a public-surface or docs concern.
- **F (Lint):** `npm run lint` green (0 warnings/errors).
- **G (Deps):** `npm outdated` clean (no safe minor/patch bumps).

**Disposition:** iterate failure-mode 3 (zero findings ≥3.0 + bold posture) would dispatch to `/expand`, but the expand cadence gate already failed this tick (7 commits < 20 since expand pass 71 at `2b03be9` today, <48h; identical signal cluster yielded 0 candidates at pass 71). Re-filing would be redundant — same disposition as passes 89/88/87/86/85/84/83/82/69. Returning cleanly; next tick re-dispatches once cadence elapses.

### 2026-06-18 (Pass 89) — Z–H walk (march→iterate dispatch)
...
**Highlights (independently re-verified this pass):**
- **Z (Critique):** CRITIQUE.md Pending empty; last critique pass 74 at `5182884` (today), 0 findings. No new pass armed (2 commits < 12-commit cadence, <24h).
- **A (Tests):** `npm test` green — 127 test files / 1821 passing (1 intentional skip). Sole `vi.spyOn(Math)` hit is the sanctioned `src/test-utils/rng.ts`. No hermetic-coverage gap ≥3.0.
...
- **C (Type-safety):** clean — `rg 'as any|@ts-ignore'` over non-test src = 0 hits; `npm run type-check` green (verify gate).
- **D (Dead code) / H (hygiene):** `git status` clean at audit start; no committed-then-orphaned files. The two commits since pass 88 are bookkeeping (critique pass 74) + a CLI-only fix (`ddfc834` — `src/CLI/hazard.cli.ts`, UI-only, excluded from build/public API, no doc/contract drift).
- **E (Docs):** every `src/` module retains a `docs/` counterpart; contracts synced last window. The lone post-pass-88 source change is CLI-only — not a public-surface or docs concern.
- **F (Lint):** `npm run lint` green (0 warnings/errors).
- **G (Deps):** `npm outdated` clean (no safe minor/patch bumps).

**Disposition:** iterate failure-mode 3 (zero findings ≥3.0 + bold posture) would dispatch to `/expand`, but the expand cadence gate already failed this tick (6 commits < 20 since expand pass 71 at `2b03be9` today, <48h; identical signal cluster yielded 0 candidates at pass 71). Re-filing would be redundant — same disposition as passes 88/87/86/85/84/83/82/69. Returning cleanly; next tick re-dispatches once cadence elapses.
...
**Highlights:**
- **Z (Critique):** CRITIQUE.md Pending empty; last critique pass 73 at `5d3f40a` (11 commits ago, < 12-commit cadence, ~<24h) so no new pass armed.
- **A (Tests):** 127 test files / 1821 passing (1 skipped) baseline green via `npm test`. Sole `vi.spyOn(Math)` hit is the sanctioned `src/test-utils/rng.ts`; the lone `it.skip` (`src/Enemy/e2e/new-enemies.engine.test.ts`) is the established intentional skip. No gap ≥3.0.
- **B (Spec-gaps):** Knowledge-Gaps only Q28 deferred (endgame, no engine work planned); specs/README recommended-order has no stale NEXT markers.
- **C (Type-safety):** clean — zero `as any` / `@ts-ignore` in non-test src; `npm run type-check` green.
- **D (Dead code):** none; working tree clean at audit start.
- **E (Docs):** every `src/` module has a `docs/` counterpart; contracts synced last window.
- **F/G/H:** ESLint green (zero warnings/errors); `npm outdated` clean (no safe minor/patch bumps); working tree clean.

**Disposition:** iterate failure-mode 3 (zero findings ≥3.0 + bold posture) would dispatch to `/expand`, but the expand cadence gate already failed this tick (3 commits < 20 since expand pass 71 at `2b03be9` today, <48h; identical signal cluster yielded 0 candidates at pass 71). Re-filing would be redundant — same disposition as passes 86/85/84/83/82/69. Returning cleanly; next tick re-dispatches once cadence elapses.

### 2026-06-18 (Pass 86) — Z–H walk (march→iterate dispatch)
...
**Highlights:**
- **Z (Critique):** CRITIQUE.md Pending empty (both critique-73 LOWs drained at 1b578ac + 134d183; no new critique pass since — 10 commits since pass 73 at 5d3f40a, below the 12-commit cadence; ~9.7h < 24h).
- **A (Tests):** 127 test files / 1821 passing (1 skipped) baseline green via `npm test`. Every top-level module carries hermetic `*.engine.test.ts` coverage; sole `vi.spyOn(Math)` hit is the sanctioned `src/test-utils/rng.ts`. The lone `it.skip` (`src/Enemy/e2e/new-enemies.engine.test.ts:56`, budget-total invariant) is the established intentional skip. No gap ≥3.0.
- **B (Spec-gaps):** the 4 `> Your answer:` blanks are spec *templates* (`00-*-template.md` + `00-how-to-use-specs.md`), not open questions; Knowledge-Gaps only Q28 genuinely deferred (endgame, no engine work planned); specs/README recommended-order has no stale NEXT markers (Phase 99 row 16 flipped NEXT→DONE at 038fdbf).
- **C (Type-safety):** clean — zero `as any` / `@ts-ignore` in non-test src; `npm run type-check` green.
- **D (Dead code):** none; working tree clean at audit start; no commented-out code blocks.
- **E (Docs):** every `src/` module has a `docs/` counterpart; no stale NEXT marker in specs/README; README/spec contracts synced last window.
- **F/G/H:** ESLint green (zero warnings/errors); `npm outdated` clean (no safe minor/patch bumps); working tree clean.

**Disposition:** iterate failure-mode 3 (zero findings ≥3.0 + bold posture) would dispatch to `/expand`, but the expand cadence gate already failed this tick (1 commit < 20 since expand pass 71 at `2b03be9`, ~2.5h < 48h; identical signal cluster already yielded 0 candidates at pass 71). Re-filing would be redundant — same disposition as passes 85/84/83/82/69. Returning cleanly; next tick re-dispatches once cadence elapses.
...
- **C (Type-safety):** clean; no @ts-ignore / as any in non-test src.
- **D (Dead code):** none.
- **E (Docs):** **FINDING (shipped this tick)** — `docs/equipment.md` claims `unique.templates.ts` holds "2 entries" in three places (line 85 source pointer, the Unique-templates table at lines 168–171 listing only `axioms-edge` + `paradox-loop`, and the line-666 template-set summary "plus 2 uniques"), but `src/Items/unique.templates.ts` `uniqueTemplates` now holds **7** entries (the 2 originals + the 5 from the 2026-06-07 content pass: `gorgon-fang`, `phoenix-mantle`, `prometheus-brand`, `oracle-eye`, `titans-girdle` — the file's own header comment reads "Seven curated Uniques"). The "2" was never refreshed after that batch. NB: line 511 `uniqueModPool (6)` is the unique-only *modifier pool* count (separate concern, verified correct). Reworded the source pointer + summary and completed the Unique-templates table with all 7 rows. Score: impact 4 × ease 9 / 10 = 3.6. **Resolved at 5a4064c.**
