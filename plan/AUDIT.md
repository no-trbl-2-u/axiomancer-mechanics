...
- [ ] **[E — docs gap] `docs/combat.md` Combat Mechanics API table missing ~13 locked-contract exports added to spec.md at `4890782`** — `isAlive`, `isDefeated`, `getHealthPercentage`, `updateEffectDuration`, `getActiveEffectModifiers`, `getEffectiveStats`, `calculateEnemyStatMultiplier`, `applyMoralMeterScaling` (functions) + `Advantage`, `CritStyle`, `CombatAction`, `CombatPhase`, `AggregatedEffectModifiers`, `EffectiveStats`, `DamageType` (types) are all in the root barrel and spec.md but absent from the `docs/combat.md` §Combat Mechanics API table. Doc-only fix. Score: 3 × 9 / 10 = 2.7. Source: iterate audit pass 123 (2026-06-27).
