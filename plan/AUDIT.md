...
- [ ] **[A — test coverage gap] `chooseSubquest` has zero dedicated hermetic tests** — `chooseSubquest(draft, chosenId)` is a public barrel export (`scripts/public-surface.expected.json:186`) but is never imported or called in any `*.test.ts` or `*.engine.test.ts` file. `hazard.engagement.engine.test.ts` covers `generateSubquestDraft`, `removeCardFromDeck`, `generateRewardOffer`, `calculateDeckScars`, `classifyDeckFocus`, and `generateDeckIdentity` — but `chooseSubquest` (the downstream verb that stamps a `chosen` candidate on the draft) was skipped. Its logic is simple (find-by-id + spread), but it is the primary consumer-facing verb after `generateSubquestDraft` and deserves direct contract coverage. Score 4 × 8 / 10 = 3.2.
