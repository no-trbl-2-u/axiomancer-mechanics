...
- [ ] **[E — docs gap] JEOPARDY/MIRACLE/BUYBACK/DELVE mechanics absent from `docs/hazard-minigame-api.md` MTG card expansion table (2026-06-26 heavy-card-library-swap)** — commit b271270 added 7 new `HazardCardDef` optional fields (`jeopardyForce?`, `jeopardyEscape?`, `firstPlayForce?`, `firstPlayEscape?`, `buyback?`, `delveForce?`, `delveEscape?`) and 4 new `HazardCardEffect` union arms (`'jeopardy'`, `'miracle'`, `'buyback'`, `'delve'`). The `docs/hazard-minigame-api.md` MTG-inspired card expansion table (`effect:` rows) was last updated at `ee5775e` covering FORETELL/ECHO/SCOUR/PURGE/BURSTMEND — the four new mechanics added in b271270 are entirely absent. Mobile developers reading the API doc to implement host flows have no description of buyback's powered-return-to-hand behaviour, jeopardy's comeback-bonus trigger, miracle's first-play bonus, or delve's discard-pile scaling. Score 4 × 9 / 10 = 3.6. Source: iterate audit 2026-06-26.

- [ ] **[E — docs gap] `docs/equipment.md` affix draw weight description omits `STATUS_AFFIX_DRAW_BIAS` multiplier (Phase 157)** — the doc says "Affix draws reuse the hidden-mod rarity scale (`AFFIX_RARITY_WEIGHTS`: `common_mod` 10 / `uncommon_mod` 3 / `rare_mod` 1)" but doesn't mention the 3× draw-weight boost applied to offensive status affixes on weapon/hands slots (via `isOffensiveStatusAffix` + `STATUS_AFFIX_DRAW_BIAS = 3` in `src/Game/game-mechanics.constants.ts:176`). The effective weights for those affixes become `common_mod` 30 / `uncommon_mod` 9 / `rare_mod` 3, making them competitive with common flat-stat affixes. A tuner adjusting `loot.statusAffixDrawBias` (registered in `src/Tuning/tunable.registry.ts:155`) has no front-door doc explaining the baseline or what the bias classifier targets. Score 3 × 9 / 10 = 2.7. Source: iterate audit 2026-06-26.
