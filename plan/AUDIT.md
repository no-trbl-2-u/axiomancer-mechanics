...
- [ ] **[E — docs drift] `getShadowedNodeOverrideKeys`, `NarrationPayload`, `runMinigameHarness` absent from bearings.md World group** — `getShadowedNodeOverrideKeys` (Phase 161 no-shadow parity guard) and `NarrationPayload` (Phase 83 narration MapEventKind type) are in the root barrel, public fixture, and docs/world.md, but absent from the bearings.md World group annotation. `runMinigameHarness` (Phase 148 harness runner) is in spec.md but not bearings.md. Similarly `DEFENSE_MULTIPLIERS`, `PASSIVE_DEFENSE_MULTIPLIER`, `MAX_EFFECT_INTENSITY`, `MAX_EFFECT_DURATION`, `FRIENDSHIP_COUNTER_MAX` are in the root barrel + docs/game.md but not spec.md Game Contracts row. Front-door contract breadcrumbs lag for these additions. Score 2 × 9 / 10 = 1.8.
