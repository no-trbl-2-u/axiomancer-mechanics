...
- [ ] **[A — test coverage gap] `dieCanPower` / `dieCanPowerCard` / `hazardCardPowerColors` have zero dedicated hermetic test cases** — public barrel exports (Hazard wildcard re-export via `src/World/Hazard/index.ts`) with documented RPS-like semantics (hex die always blocks; gold die always powers; color match required otherwise). `dieCanPowerCard` additionally handles two-tone `colors` arrays present on ~15+ cards in `hazard.content.ts`. No describe block in any `Hazard/e2e/*.engine.test.ts` exercises these three helpers directly. Score 4 × 8 / 10 = 3.2. Source: iterate audit 2026-06-26.
