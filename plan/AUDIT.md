...
- [ ] **[H — walkthrough drift] DIV-MECH-005: fishing-village-exploration.json still drives legacy combat tab; update to Hazard-Pattern Combat via `combat.cli.ts`** — Update `automation/scripts/walkthroughs/fishing-village-exploration.json` (and `.goal.md`) to trigger `npm run combat --auto` for the fv-15 encounter instead of legacy combat-tab defend rounds; update the `src/Game/e2e/spec08.engine.test.ts` encounter-coverage comment to note Hazard-Pattern Combat is the live map-encounter surface; confirm `automation/scripts/walkthroughs/README.md` reflects the change. Automation/doc scope only — no engine source changes required (walkthrough calls the standalone `npm run combat` CLI, bypassing `game.cli.ts` routing). Unblocked by Phase 165 (`combat.cli.ts`, `3ed4755`). Closes the last open row in `divergences.md` (DIV-MECH-005). Score: 3 × 9 / 10 = 2.7. Source: oversight 2026-06-25 (T: iterate tick); critique-87 LOW-3; divergences.md DIV-MECH-005.
