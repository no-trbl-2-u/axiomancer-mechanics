...
- [ ] **[E — docs drift] `confirmHazardForetell` and MTG-expansion keyword fields absent from `docs/hazard-minigame-api.md`** — commit `181696b` added `confirmHazardForetell` to the Hazard barrel (resolves a `foretell-pending` state that the API guide's state machine diagram does not show) and added 8 new `HazardCardDef` optional fields (`foretellScour`, `foretellDrawCount`, `echoPerCardForce`, `echoPerCardEscape`, `purgeDrawCount`, `burstMendBase`, `burstMendPowered`, `burstPerUnspentDieEscape`) plus 3 new `HazardCardEffect` enum values (`foretell`, `echo`, `scour`). None appear in the `docs/hazard-minigame-api.md` Package surface import list or the state machine block. A mobile developer checking `session.phase === 'foretell-pending'` without knowing to call `confirmHazardForetell` after the player reorders will stall. Score: 4 × 9 / 10 = 3.6.
