...
> Last pass: 2026-06-02 at commit d740694
> Pass count: 29
-->
...
- signal: Phase 101 (`8ccb61b`) tuned exactly ONE enemy (Coastal Tyrant mercy route); Phase 104 (`99e5174`) stood up two reproducible reference-playtest probes (early-game level-1 vs easiest fishing-village enemies; endgame max-level vs late-game/boss) that emit survivability / rounds-to-resolve / damage-dealt-vs-taken bands. The probes exist but only the Tyrant has been tuned against them. The user's recurring top concern — "difficulty is all over the place / no order" (playtest jot #89, restated through oversight-26/27) — is a roster-wide problem, not a one-boss problem. The infrastructure to measure it now ships; nothing has consumed it across the broader roster.
- scope: One phase, 3 commit units. **Unit 1 — measure**: run the Phase 104 early-game + endgame probes across the full fishing-village + northern-forest enemy roster (normals + elites + bosses); record a per-enemy pass/out-of-band table (which enemies resolve too fast, too slow, or are unwinnable/trivial vs the intended bands). **Unit 2 — tune**: adjust HP / damage / resist / stance-bias on the enemies that fall outside band, smallest-change-first; keep authored alignment pins (Phase 45) and skill rotations (Phase 49/57) intact; pin the post-tune bands with a hermetic playtest test per touched enemy. **Unit 3 — docs**: `docs/enemy.md` difficulty-band notes + `automation/playtest/NEXT_STEPS.md` roster-tuning record + `CHANGELOG.md [unreleased] ### Changed`.
...
- blocked-by: None — Phase 104 probes are shipped. Wants a light user-attended confirmation of the intended bands at oversight time (what "too punishing" / "trivial" mean numerically) if the bands aren't already locked.
...
- recommended-slot: next stabilisation slot — pairs with the difficulty-curve-doctrine candidate below (which gives it a target) and the mid-game probe candidate (which extends its measurement coverage).
...
- signal: Phase 104 (`99e5174`) shipped two reference probes — early-game (level 1) and endgame (max level) — leaving the mid-game band (≈ level 7-8, the northern-forest escalation tier where Phase 49 enemy-skill casters and elite enemies first appear) unmeasured. The difficulty curve is exactly where the early→endgame gap is widest and least observed; the user's "all over the place" report most plausibly lives in this untested middle.
- scope: One small phase, 2 commit units mirroring Phase 104's pattern. **Unit 1 — fixture**: author a mid-game reference Playtest fixture in `src/Playtest/` (a level-7-8 character with Tier 1+2 skills + mid-tier gear) against the northern-forest elite roster, runnable via the existing playtest runner + report. **Unit 2 — test + docs**: hermetic playtest test pinning the fixture resolves and emits a report with the same band fields; `docs/playtest.md` + `automation/playtest/NEXT_STEPS.md` document it as the third canonical probe between early-game and endgame.
- unblocks: Completes the early→mid→endgame measurement triad so the roster-tuning candidate can tune the middle of the curve, not just the ends. Closes the most likely blind spot in the difficulty-legibility work.
- blocked-by: None — reuses the Phase 104 harness + fixture pattern directly.
- score: urgency 5 × value 5 / 10 = 2.5 (medium urgency — fills a measurement gap; medium value — mid-game coverage; high ease — direct mirror of the shipped Phase 104 fixture pattern). Aggressive 2.5+ threshold catches this.
- recommended-slot: alongside or just after the roster-wide tuning candidate; feeds its Unit 1 measurement.
