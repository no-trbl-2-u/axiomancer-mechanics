...
### Candidate: Befriending mechanic v2 — richer than friendshipCounter capping
- signal: user direction at oversight 2026-05-20 (ninth-of-session): "For the boss-tier befriending, we'd need a whole new befriending mechanic in order to balance that." Phase 36 shipped the simple mechanic (both-defend rounds increment `friendshipCounter`; cap → friendship outcome). Phase 60 added per-enemy reward content; Phase 62 added quest-branch wire-in. The current mechanic works for low-tier enemies whose passive-defense damage output is survivable, but **doesn't scale**: (a) for normal enemies the path is too easy (just defend-defend), removing the choice's weight; (b) for boss-tier enemies the path may be impossible — bosses out-damage the player's passive-defense recovery, so the friendship-counter cap is unreachable before HP runs out. A richer mechanic gives the player meaningful agency in befriending AND makes boss-tier befriending mechanically viable.
- scope: One phase, design-heavy. Brief at dispatch picks ONE of (or a sub-combination of) these design directions:
  - **(a) Per-enemy `BefriendabilityConfig`** — opt-in field on `Enemy` describing the befriend predicate (rounds threshold, HP gate, required stance combos, required skill use). Phase 36's "both-defend caps the counter" becomes the default fallback when the field is absent.
  - **(b) Health-threshold reverse-gate** — once the enemy is brought below X% HP without killing, friendship eligibility opens; further defends accelerate the counter. Re-frames befriend as "show your strength, then offer peace."
  - **(c) Skill-driven friendshipCounter increments** — new Tier 2 "calming" skill payloads that increment the counter directly when cast. Pairs with the Tier 2 synergy candidate; ships a `incrementsFriendship?: number` skill payload field.
  - **(d) Multi-stage befriend** — round-by-round buildup with possible failure; e.g. specific stance choices each round advance toward befriend, wrong stance choices reset. Most narratively expressive; most engine work.
  - **(e) Combat-resource cost** — spend X mind / heart tokens per round to convey willingness; the cost makes befriending a real trade-off.
- unblocks: Boss-tier befriendable enemy candidate (currently in Pending, score 2.4) becomes mechanically viable. The simple Phase 36 mechanic stays as the fallback for low-tier enemies that don't opt in. Future content phases (befriendable-enemy arcs per-region) get a richer authoring surface.
- blocked-by: None on the engine side. Wants a **user-attended design pass via `/oversight`** before brief drafting — the 5 design directions above are mutually exclusive in some shapes (a single phase ships one of them, not all).
- score: 6 × 4 / 10 = 2.4 (medium-high impact — unblocks an authored content arc + tightens an existing mechanic; medium ease — design call + 2-3 commit units of engine work depending on direction picked).
- recommended-slot: queue ahead of Boss-tier befriendable enemy (which is now blocked-by this candidate per the oversight-9 framing).
...
- blocked-by: **Befriending mechanic v2** (candidate above, filed at oversight-9 2026-05-20). The user's direction: "For the boss-tier befriending, we'd need a whole new befriending mechanic in order to balance that." The current Phase 36 mechanic (both-defend caps `friendshipCounter`) doesn't scale to boss-tier — bosses out-damage the player's passive-defense recovery, so the counter cap is unreachable before HP runs out. Resolve the design call on Befriending mechanic v2 first; the boss-tier reward authoring then ships against the richer primitive.
- score: 4 × 6 / 10 = 2.4
- recommended-slot: directly after Befriending mechanic v2 ships. The reward content this candidate ships consumes whatever new primitive Befriending mechanic v2 lands (e.g. if v2 picks the per-enemy `BefriendabilityConfig` option (a), this candidate's boss-tier enemy gets its config authored at the same time).
