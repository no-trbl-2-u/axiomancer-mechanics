...
> Last pass: 2026-05-21 at commit 2682c6d
> Pass count: 18
-->
...
### Candidate: `FriendshipReward.alignmentDelta` extension (closes Spec 14 Q4)
- signal: Spec 14 Q4 ("friendship-victory ↔ alignment cube intersection") was filed as "orthogonal by design" at Phase 58 / Phase 63; the boss-tier befriendable-enemy candidate's scope (`alignmentDelta?: Partial<PhilosophicalAlignment>` on `FriendshipReward`) treats this as opt-in per encounter. Phase 68 unblocks the boss-tier work but the typed surface extension is its own additive change and a cleaner standalone phase: it lets ANY befriendable enemy nudge the player's alignment without waiting for boss-tier reward authoring. Pairs naturally with Phase 43's `alignmentDelta` authoring pattern on dialogue choices + map-event pools (same `applyAlignmentDelta` clamp helper).
- scope: One phase, 2 commit units. **Unit 1 — engine primitive.** Extend `FriendshipReward` with `alignmentDelta?: Partial<PhilosophicalAlignment>`; `store.endCombat()` threads the delta through `applyAlignmentDelta` on the `outcome === 'friendship'` branch (after the existing items / xpBonus / narrative / flagSet threads). `CombatEndReport.friendshipReward` gains an optional `alignmentShift?: PhilosophicalAlignment` field surfacing the post-clamp pin (mirrors how `applyDialogueChoice` returns the new cell). Fixture bump: 162 → 163 types. **Unit 2 — content + e2e + docs.** Author `alignmentDelta` on one or two existing befriendable enemies (MournfulGull → small `outlook: +5` per the gull's wistful tone? — exact deltas at brief time); hermetic e2e extends `befriend.engine.test.ts` with the alignment-shift cases (cold-cell + clamp-edge); `docs/enemy.md` Befriendable-enemies table gains an `alignmentDelta (Spec 14 Q4)` column; `specs/14-philosophical-alignment.md` Q4 flipped to "Resolved at Phase NN"; `CHANGELOG.md [unreleased] ### Added` entry.
- unblocks: closes Spec 14 Q4 fully; provides the typed surface for the Boss-tier befriendable enemy candidate's `alignmentDelta` authoring (mentioned as a deferred sub-scope in that candidate). Future befriendable-enemy arcs get a richer reward toolkit.
- blocked-by: None. Phase 43's `applyAlignmentDelta` helper is the canonical implementation reference.
...
- recommended-slot: queue ahead of Boss-tier befriendable enemy so the boss-tier candidate consumes this primitive cleanly.
...
- blocked-by: None — **Phase 68 shipped at commits `99a0cc9` + `73105dc` + `79722ec` (2026-05-20)**. Coastal Tyrant's `befriendabilityConfig` (`hpGate: { belowPct: 0.4 }`, `requiredStances: ['heart']`, `roundsThreshold: 5`) is live in `src/Enemy/enemy.library.ts:343-347`. This candidate now adds only the reward-content side (`friendshipReward` extension with `alignmentDelta?: Partial<PhilosophicalAlignment>` + multi-paragraph narrative + boss-tier item rewards). If the `FriendshipReward.alignmentDelta` extension candidate (filed at expand-17 above) ships first, this candidate's scope shrinks further to pure narrative + item authoring.
- score: 4 × 6 / 10 = 2.4
- recommended-slot: directly after Phase 68 ships. The reward content this candidate ships consumes the `BefriendabilityConfig` primitive (already wired on Coastal Tyrant by Phase 68 Unit 2) — this candidate's scope shrinks to pure reward-content authoring + optional `FriendshipReward` shape extension.
