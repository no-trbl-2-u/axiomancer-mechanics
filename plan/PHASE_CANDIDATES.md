...
> Last pass: 2026-05-28 at commit 1378b42
> Pass count: 27
-->
...
### Candidate: Coastal Tyrant mercy-route tuning + playtest policy/report depth
- signal: Tobin-facing automated playtest evidence now says the boss friendship route is mechanically legible but not reliably reachable. The pinned harness note (`automation/playtest/NEXT_STEPS.md`) already framed the central design choice: "wound then spare" versus "nonviolent patience." A fresh oversight rerun after Phases 91-95 sharpened the charge: 25 runs of `late-game-coastal-tyrant` produced 9 victories, 0 defeats, 0 friendships, and 16 timeouts (36% victory / 64% timeout / 0% friendship). The pure `friendship` policy built counter up to 36 but never crossed the HP gate; average final enemy HP under that policy remained 455. The earlier resume-pass report had mixed/random policies surfacing friendship, so the regression may be balance drift from recent mechanics, policy intelligence lag, or the route's intended doctrine being under-specified.
- scope: One design-attended tuning phase, 3 commit units. **Unit 1 — evidence refresh.** Add a `mercy` policy that damages The Coastal Tyrant until the authored HP gate is reachable, then shifts to heart/defend to test the "wound then spare" doctrine directly; keep pure `friendship` as the nonviolent patience negative control. Extend the report with compact HP-gate traces (first round below gate, rounds spent eligible, final gate reason). **Unit 2 — tuning decision.** Based on the refreshed run matrix, pick one doctrine: (a) keep wound-then-spare and tune policy/UX evidence only; (b) add an alternate surrender predicate for nonviolent patience; or (c) reduce defensive stall / durability so mercy routes resolve before timeout. **Unit 3 — docs + guards.** Update `automation/playtest/NEXT_STEPS.md`, `docs/combat.md` friendship-path prose if doctrine changes, and add a hermetic playtest test asserting the new policy/report fields are produced.
- unblocks: Gives Tobin a clean proving ground for boss mercy before expanding to agentic playtesters or adding more late-game boss friendship content. Prevents Coastal Tyrant from teaching a false lesson: that mercy is possible in lore but unavailable in play.
- blocked-by: Phase 96 and Phase 97 may continue first; this candidate should not reorder the current queue. Best slotted after the mobile-contract fixes unless T wants balance doctrine before new content.
- score: 6 × 5 / 10 = 3.0 (high impact — boss friendship doctrine and timeout rate are core RPG feel; medium ease — additive policy/report work plus one design call, with possible small tuning change).
- recommended-slot: after Phase 97, before expanding to Northern-forest / second-continent content or hiring The Kid for UI-level playtesting.
...
- signal: AUDIT.md GH#74 — BattleLogEntry fields undefined at runtime despite required types
- scope: Audit the BattleLogEntry type vs runtime contract, fix the divergence causing mobile crashes on boss-skill killing blows. Combat resolution paths leave required fields undefined while types declare them as non-optional.
- unblocks: Mobile combat stability, eliminates crashes on boss encounters
- blocked-by: none (ready to iterate)
- score: 7 × 6 / 10 = 4.2
...
- unblocks: Meaningful loot progression, build diversity through itemization
- blocked-by: Item rarity system (Spec 05c) for the modifier integration points
- score: 7 × 10 / 10 = 7.5
- recommended-slot: after Spec 05c phase

### Candidate: Set items system (Spec 05e)
- signal: specs/05e-set-items.md unstarted + braindump 2-4 piece set design + Spec 05e was explicitly promoted as Phase 54 but skipped pending design
- scope: Multi-piece equipment sets with incremental bonuses (2-piece, 3-piece, etc). Set bonuses use existing ResourceInteraction/passiveEffects/statModifiers shapes, no new resolver needed. Ship 2-3 initial sets using base templates.
- unblocks: Endgame collection goals, strategic equipment choices beyond individual items
- blocked-by: Item rarity system + modifier catalogue for template foundation
- score: 7 × 10 / 10 = 7.0
- recommended-slot: after Spec 05d phase

### Candidate: Coastal Tyrant mercy tuning (user-filed)
- signal: Recent commits show oversight filed "Coastal Tyrant mercy tuning candidate" at commit 2dc6398
- scope: Balance pass on Coastal Tyrant boss encounter. The oversight commit suggests the fight may be too punishing and needs mercy mechanics or stat adjustments to improve new player experience.
- unblocks: Better new player onboarding, reduced frustration with first boss encounter
- blocked-by: none (content tuning, no dependencies)
- score: 6 × 5 / 10 = 3.0
- recommended-slot: after Phase 97
