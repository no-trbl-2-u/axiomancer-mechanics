...
> Last pass: 2026-05-23 at commit 7b9e23e
> Pass count: 22
-->
...
### Candidate: `previewTemplateAtAllRarities` batch helper (Phase 75 follow-up)
- signal: Phase 75 (`6e04b50` + `2d2d3f4` + `d9e8a17`) shipped `previewTemplateAtRarity(templateId, rarity, playerLevel, rng?): Equipment | undefined` as a single-cell preview surface. The Phase 75 brief's Follow-ups section named a candidate "convenience wrapper that returns `Record<ItemRarity, Equipment | undefined>` for a single template" — useful for mobile UI views that render every-rarity-for-this-template comparison strips (e.g. an item-detail tooltip showing "Common / Uncommon / Rare / Unique" stat ladders side-by-side). Today mobile would call `previewTemplateAtRarity` four times per template; a batch helper saves the boilerplate + signals intent.
...
- score: 4 × 8 / 10 = 3.2 + 0.5 user-source-adjacent (closes the Phase 75 follow-up the user might naturally want next) + 1.5× items bias = **5.55**.
- recommended-slot: any time after Phase 75. Smallest natural follow-up since it's a literal one-function wrapper around the just-shipped primitive.
