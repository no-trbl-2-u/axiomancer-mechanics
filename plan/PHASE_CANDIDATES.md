...
### Candidate: Coastal Tyrant friendship HP-gate tuning (playtest timeout signal)
- signal: Oversight 2026-05-26 ran the default deterministic playtest harness on current `origin/main` after PR #75 merged (`npm run playtest -- --out-dir /tmp/axiomancer-playtest-report`). Scenario `late-game-coastal-tyrant` produced 25 runs: 11 victory / 8 friendship / 0 defeat / 6 timeout (winRate 44%, friendshipRate 32%, timeoutRate 24%). The friendship policy built enough counter (`maxFriendshipCounter: 36`) but still timed out because the Coastal Tyrant HP gate stayed unmet at average final enemy HP 455. Replay seeds include `late-game-coastal-tyrant-v0:2`, `:3`, `:9`, `:15`, `:20`. T accepted the recommendation to keep Phase 90 as next and file this as a post-release balance candidate rather than interrupting the v0.12.0 cut.
- scope: One balance-audit phase after Phase 90. Unit 1 — rerun `automation/playtest/scenarios/late-game-coastal-tyrant.json` and inspect the friendship-policy transcripts for how often `friendshipCounter` satisfies the threshold before HP falls below `CoastalTyrant.befriendabilityConfig.hpGate.belowPct`. Unit 2 — choose one conservative adjustment path: lower the HP gate, reduce the required rounds, improve the friendship policy's damage/counter mix, or author a calming-skill / damage-resist dependency if Phase 91/93 has shipped and gives a cleaner lever. Unit 3 — update the scenario expected findings / docs / CHANGELOG and add a hermetic or harness-level assertion that the timeout rate stays below an agreed ceiling while defeat remains non-dominant.
- unblocks: Converts the new automated playtester from infrastructure into design judgment. Gives Tobin-grade evidence for the boss-tier peaceful route without derailing the release ceremony.
- blocked-by: Phase 90 release cut should ship first per oversight 2026-05-26. Phase 91 (`incrementsFriendship?: number`) may provide a better lever if shipped before this candidate is promoted, but is not required.
- score: 6 × 6 / 10 = 3.6 (medium-high impact — live playtest shows a 24% timeout scar on a marquee boss friendship route; medium-high ease — narrow scenario, one enemy, one predicate matrix; post-release so it does not disturb Phase 90).
- recommended-slot: after Phase 90, or after Phase 91 if the calming-skill primitive becomes the preferred fix lever.
