...
- blocked-by: **Phase 68 — Befriending mechanic v2** (promoted at oversight-11 2026-05-20 under design path (a) per-enemy `BefriendabilityConfig`). Phase 68 Unit 2 already authors Coastal Tyrant's `befriendabilityConfig` (`hpGate: { belowPct: 0.4 }`, `requiredStances: ['mercy']`, `roundsThreshold: 5`); this follow-up adds the reward-content side (`friendshipReward` extension with `alignmentDelta?: Partial<PhilosophicalAlignment>` + multi-paragraph narrative + boss-tier item rewards) once the predicate primitive lands.
- score: 4 × 6 / 10 = 2.4
