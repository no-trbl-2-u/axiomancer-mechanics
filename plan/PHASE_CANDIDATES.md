...
### Candidate: Post-GH#65 per-foe content sweep (Phase 71 + Phase 73 missing-enemy authoring)
- signal: Phases 71 (per-foe aftermath narrative prose — `finalBlowLines` / `pactLines` / `causeLines`) and 73 (Codex `journalEntry`) each authored content on only the 3 currently-befriendable enemies (MournfulGull + HollowEyedBeggar + CoastalTyrant). Both phases explicitly named the remaining 13 enemies as a follow-up content sweep (`enemy.library.ts` roster: TidepoolCrab, SeaMistWisp, LullabyMoth, Disatree_01, WetHound, ForestSprite, ArgumentativeCrow, TideflukeReaver, HushWraith, HollowSaint, TheDisagreement, EchoOfPyrrhonia, Sandbag_01). Mobile presenter currently falls back to its `derive*Phrase` helpers for un-authored enemies (Phase 71 D8); codex unlocks simply don't fire on the friendship paths for un-authored enemies (Phase 73 D9). With Phase 71/72/73 just shipped (`4dee78e`), the engine surface is complete and content authoring is the only remaining work to bring the mobile aftermath panel to full coverage across every encounter.
...
- unblocks: Mobile aftermath panel renders engine-authored chronicle prose for every encounter (no presenter-derived fallback for the player-visible majority of fights). Codex tab populates as the player traverses content rather than only on the 3 befriendable encounters. Establishes the content-sweep pattern as a follow-up shape for any future per-foe surface (e.g. if a hypothetical Phase 74 adds `examineLines` or `restLines`, the sweep can mirror this phase's shape).
- blocked-by: None. Phase 71 + Phase 73 engine surfaces are shipped; pure content authoring.
- score: 5 × 6 / 10 = 3.0 (medium impact — closes the consumer-facing coverage gap; high ease — content-only against fully-shipped engine fields; no decision pass needed beyond per-enemy voice work).
- recommended-slot: any time after Phase 73 (already shipped). Pairs naturally with the Walkthrough catalog expansion candidate since post-sweep walkthroughs can pin the per-foe coverage.

### Candidate: CLI run-loop + Codex integration (Phase 72 + 73 consumer surfaces)
...
### Candidate: `previewTemplateAtRarity` helper (mobile item-library mod-visibility)
- signal: User jot at `b5c8165` ("modifiers not implemented; can we fast-track those?") refined at oversight-15 2026-05-23 against verified state: modifiers ARE implemented (Spec 05d shipped pre-loop — `modifier.catalogue.ts` 537 lines + `rollModifiers`/`resolveModifiers` real at `item.factory.ts:140,:197` wired into `dropItem` at `:333-334` + 369-line e2e + `docs/items.md` Modifier-catalogue section + Spec 05d acceptance all `[x]`). The real gap is at the consumer boundary: `equipmentTemplates` (exported on `src/index.ts:106`) carries only `baseStatModifiers` by design (the file's top comment confirms — rolled mods only exist on runtime `Equipment` instances from `dropItem`). Mobile UI rendering an item library from `equipmentTemplates` shows zero modifiers on every entry — mechanically truthful but player-experience misleading. There is no engine surface for "what would this template look like rolled at Uncommon/Rare/Unique at player level N?"
- scope: One phase, 2 commit units. **Unit 1 — engine primitive.** New `previewTemplateAtRarity(templateId: string, rarity: ItemRarity, playerLevel: number, rng?: () => number): Equipment | undefined` helper in `src/Items/item.factory.ts` (or sibling). Default `rng` to a fixed seed (e.g. `() => 0.5` per the Phase 70 Coastal Tyrant deterministic-drop pattern) so previews are reproducible per (template, rarity, level) tuple. Implementation reuses the existing `dropItem` machinery with the rng + rarity pinned. Re-exported through `src/Items/index.ts` + top-level barrel. Hermetic e2e at `src/Items/e2e/preview-template.engine.test.ts` covering: (a) returns Equipment with rolled modifiers matching the rarity tier (0/1/2/3 mods for Common/Uncommon/Rare/Unique); (b) deterministic per fixed-rng-seed; (c) returns `undefined` for unknown templateId; (d) requiredLevel-gated value-tier resolution. Public-surface fixture bump (+1 runtime export). **Alternative shape** (brief picks at dispatch): `getTemplatePreviewModifiers(template, rarity, playerLevel, rng?): RolledModifier[]` (returns just the mod list; UI composes the display) — slightly less convenient for mobile but more composable. **Unit 2 — docs + CHANGELOG.** `docs/items.md` Modifier-catalogue section extended with a "Previewing rolled mods (library / catalog views)" subsection naming the helper + the deterministic-preview convention. `docs/api.md` Items block extended. `README.md` Items row extended. `CHANGELOG.md [unreleased] ### Added` entry.
- unblocks: mobile item-library view can render `equipmentTemplates` × `ItemRarity` tiers with rolled mods visible per cell. Same engine-asks-from-mobile-consumer-side pattern as GH#65 — engine exposes the typed surface, UI consumes it. Future inventory-detail / loot-preview / vendor-stock surfaces can call the same helper. Closes the user-jot finding cleanly (the row stays Pending in CRITIQUE until this candidate promotes; or the candidate-promotion is itself the drain).
- blocked-by: None — all consumed engine surface (templates / rarity / rollModifiers / resolveModifiers / dropItem) is shipped.
- score: 5 × 6 / 10 = 3.0 (medium impact — unblocks a real mobile UI surface + closes a user-flagged gap with +0.5 user-source bump; medium-high ease — additive helper that reuses existing `dropItem` machinery; small public-surface footprint).
- recommended-slot: any time after Phase 71/72/73. If the user wants this fast-tracked (per the jot framing), it can be promoted ahead of the existing queue at the next oversight.
...
### Candidate: Walkthrough catalog expansion — Phase 65 / 66 / 68 coverage
