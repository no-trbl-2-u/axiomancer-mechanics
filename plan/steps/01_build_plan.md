...
- [ ] Phase 76 — `previewTemplateAtAllRarities` batch helper (Phase 75 follow-up) per oversight-18 2026-05-23 promotion (user picked at Q1; smallest natural follow-up to Phase 75; bias-adjusted highest in pool at 5.55). New `previewTemplateAtAllRarities(templateId: string, playerLevel: number, rng?: () => number): Record<ItemRarity, Equipment | undefined>` in `src/Items/item.factory.ts`; internally calls `previewTemplateAtRarity` four times (once per `ItemRarity` value: common / uncommon / rare / unique). Returns the full record shape; `undefined` cells signal "this template can't roll at that rarity at this level". Re-exported through `src/Items/index.ts` + top-level barrel (+1 runtime export — fixture 236 → 237). Two commit units: (1) engine helper + hermetic e2e at `src/Items/e2e/preview-template.engine.test.ts` extended (3 new cases — returns all-4-rarity record per template; level-too-low yields all-undefined record; unique template returns only `unique` populated); (2) docs — `docs/items.md` Modifier-catalogue "Previewing rolled mods" subsection extended; `docs/api.md` Items block extended; `README.md` Items row extended; `CHANGELOG.md [unreleased] ### Added` entry; `plan/bearings.md` Items block extended (the Phase 75 fold-in from iterate-`8c94ada` gets a Phase 76 sibling line). **No engine math change** — thin wrapper around Phase 75 primitive. Source: candidate filed at expand-22 `4826c37`; promoted at oversight-18 (this commit). Score 4×8/10 = 3.2 + 0.5 user-source-adjacent + 1.5× items bias = **5.55**.
