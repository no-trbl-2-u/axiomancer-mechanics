...
- [ ] Phase 77 — Audit: base-action token generation (Spec 04 token economy) per oversight-18 2026-05-23 promotion. User-flagged at oversight-18 with explicit "Audit that feature." framing — suspected regression "base attack/defend may not be generating the tokens it was supposed to". Per Spec 04 Q-on-basic-action-grants the CombatResources system has basic actions generating tokens (miss=1, hit=3, defend=5); audit verifies actual code behaviour matches. Two commit units (Unit 2 conditional): (1) audit — walk `src/Combat/combat.resolver.ts` + the basic-action path (likely `scenario.ts` or sibling) + `specs/04-skills-engine.md` Q-on-basic-action-grants + the hermetic tests pinning token generation (likely in `src/Combat/e2e/` or `src/Skills/e2e/`); produce a per-action audit table (`attack-hit` / `attack-miss` / `defend` × {spec / code / tests / drift?}); drop findings in `plan/CRITIQUE.md` Pending as MED rows if drift exists, or in `plan/AUDIT.md` Done as "verified" if it clears. (2) conditional fix — if audit surfaces a real bug, patch the resolver + adjust hermetic tests; per-bug commit. Pre-work for the skills-always-land mechanic shift candidate (need confirmed token generation before changing skill resolution semantics). Source: candidate filed at oversight-18 (this commit). Score 6×7/10 = 4.2 (high-medium impact + high ease).
