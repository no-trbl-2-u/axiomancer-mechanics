...
- [ ] Phase 53 — Public-surface snapshot artifact + diff tool: automates the engine-public-surface diff that this session hit twice (mobile upgrade doc + getCoastalMap removal). 2-3 units. Unit 1 — `scripts/snapshot-public-surface.mjs` walks `dist/index.d.ts` and emits a deterministic-sorted JSON listing every exported name + kind (`type` / `function` / `const` / `enum`) + arity-or-shape hint for functions and types. Output committed as `scripts/public-surface.expected.json`. Unit 2 — `scripts/deploy-check.mjs` post-build assertion re-runs the snapshot and fails the gate on any drift from the fixture (replaces the Phase 50 hand-maintained `public-barrel.engine.test.ts` for surface drift; that test stays for runtime behaviour). Unit 3 — `scripts/diff-public-surface.mjs <ref-A> <ref-B>` pretty-prints Added / Removed / Changed between two git refs, ready to feed Phase 52's CHANGELOG bullets. Promoted via `/oversight` 2026-05-19 (same oversight as Phase 52) from `expand pass 11` candidate. Score 5×6/10 = 3.0.
