...
- [ ] Phase 43 — Alignment-shifting authoring sweep on MapEvent + dialogue content: promoted via `/oversight` 2026-05-16 as the top-scoring candidate after Phase 42 ship. Three units. Unit 1 — type-extend MapEvent + dialogue choice payload shapes with `alignmentDelta?: Partial<PhilosophicalAlignment>` and thread through `resolveMapEvent` + `applyDialogueChoice` (mirrors the existing `moralDelta` path). Unit 2 — author first-pass alignment deltas across `src/World/Continents/Coastal-Village/maps.ts` + the Old Marrow / Fishing Village dialogue trees; calibrate to the same `±1..±5` band moralMeter uses; `±10` defining choices reserved for endgame. Unit 3 — hermetic e2e drives a fishing-village walk through a known-alignment-shifting choice and asserts the cell moves; refresh `docs/philosophy.md` with an "Authoring deltas" subsection. Closes Phase 42 Follow-ups #1; makes the alignment cube observable in actual play. Score 6×7/10 = 4.2.
...
- [ ] Phase 45 — Enemies-by-alignment AI tuning: promoted via `/oversight` 2026-05-16 (promote-multiple Phase 42 follow-up sequence). Two units. Unit 1 — add optional `philosophicalAlignment?: PhilosophicalAlignment` to `Enemy`; backfill every authored enemy in `src/Enemy/enemy.library.ts` with a cell pin (Coastal Tyrant = `faith-pessimistic-transcendent`, Tidepool Crab = `mid-mid-individual`, etc.). Unit 2 — `decideEnemyAction` accepts an optional alignment-tuner that biases the basic-action choice by outlook bucket (Pessimistic → defend/friendship-stall, Optimistic → attack). Hermetic e2e pins the bias delta against `mockSequentialRng`. Update `docs/enemy.md`. Sequenced after Phase 44 so the player-side payload library exists before enemies draw from it. Score 5×6/10 = 3.0.
