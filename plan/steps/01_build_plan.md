...
- [ ] Phase 73 — Codex / journal-entry surface on parley outcomes (mobile GH#65 ask 3, score 4.0) per oversight-14 2026-05-22 promotion. Unblocks the mobile `<CombatFriendshipPanel>` "A NEW ENTRY" card which currently never mounts because the engine doesn't expose any codex surface. New Codex slice on `GameState`: `codex: { unlockedEntries: string[] }` (additive; save migration defaults to `[]`). New per-foe optional `journalEntry?: { id: string; title: string; body: string }` (or richer shape — brief at dispatch picks between flat string vs structured; structured pairs better with future Codex-tab UI). New `unlockCodexEntry(entryId: string): GameState` reducer-level action + auto-firing wiring on `store.endCombat()` `outcome === 'friendship'` branch (after the existing items / xpBonus / narrative / flagSet / alignmentDelta threads). `CombatEndReport.friendshipReward` gains an optional `codexEntryUnlocked?: { id: string; title: string }` field (mirrors the Phase 69 `alignmentShift` surfacing pattern). De-duped via `includes()` on the `unlockedEntries` array. Three commit units: (1) engine primitive + Codex slice + save migration + reducer action; (2) content authoring — `journalEntry` on the 3 authored befriendable foes (CoastalTyrant magistrate-fallen-priest chronicle; MournfulGull wistful-empathy chronicle; HollowEyedBeggar reversal-of-begging chronicle); (3) hermetic e2e (`src/Game/e2e/codex.engine.test.ts` — unlock on friendship + de-dupe + foes without journalEntry no-op + victory outcome doesn't unlock) + `docs/api.md` Game-state Codex block + `docs/combat.md` Friendship Path codex-entry note + `CHANGELOG.md [unreleased] ### Added`. Mobile callsite: `<CombatFriendshipPanel>` NEW ENTRY card reads `report.friendshipReward.codexEntryUnlocked` and mounts when populated. Source: GH#65 ask 3 (filed 2026-05-22, scored 4.0).
