...
- [ ] Phase 67 — CLI overview / quick-start doc (user write-in at oversight-10): "Can I get an overview doc of what's been implemented and how I can test it with the CLI". Author `docs/quickstart.md` as a single-page entry-point covering: what's shipped per module, how to run the CLI (`npm run game` + tabs + scripted mode), walkthrough catalog (11 at `automation/scripts/walkthroughs/`), key in-game flows (combat with friendship path, map exploration, save/load), verify gates, and pointers to deeper docs. Pure docs work — no engine touch; assembles existing material. Score 5×7/10 = 3.5. See `plan/PHASE_CANDIDATES.md` Promoted section for full scope.
