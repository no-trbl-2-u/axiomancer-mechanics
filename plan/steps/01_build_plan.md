...
- [ ] Phase 55 — PersistenceAdapter ergonomics (Phase 50 follow-up): closes the last loose end of the GH#64 engine handoff. Mobile re-declares `PersistenceAdapter` locally rather than importing it from `axiomancer-mechanics` due to "specific ergonomic friction" with the engine's exported interface (per `axiomancer-mobile/docs/engine-team-handoff-2026-05-16.md` §Issue 3). 1-2 units. Unit 1 — read the mobile-side local re-declaration in `axiomancer-mobile/state/persistence/asyncStorageAdapter.ts` and diff against the engine export in `src/Game/persistence/types.ts`; identify the specific friction; land the engine-side change that makes the local re-declaration unnecessary. Unit 2 — extend the public-barrel hermetic test to assert the `PersistenceAdapter` export shape matches what mobile expects (mobile-side signature as the source of truth). Document in CHANGELOG.md on the next `[unreleased]` heading post-0.10.1 publish. Promoted via `/oversight` 2026-05-20 (fifth oversight of the session) from `expand pass 11` candidate. Score 3×6/10 = 1.8.
