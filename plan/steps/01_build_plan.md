...
- [ ] Phase 66 — Tier 2 synergy skills (5 patterns + `whenEffectAtLeast` predicate clause): promoted via oversight 2026-05-20 (tenth-of-session) after the user-attended design pass picked both the engine primitive shape (predicate-clause on existing payloads, smallest engine touch) and the content scope (4 candidates + 1 write-in resonance-detonation variant). 5 skills to author: cross-stance duration amp (heart × body DoT), buff type-swap (Body Thorns → Heart Bat Swarm), cross-stance intensity amp (mind × heart buff), resonance burst (consume opposing debuff for damage), resonance detonation (consume full combat-resource pool + clear all effects for one-shot massive payload). Phase brief at dispatch fixes per-skill stance / cost / cell mapping. Three units: engine primitive + 5-skill authoring + e2e/docs/CHANGELOG. Score 6×4/10 = 2.4. See `plan/PHASE_CANDIDATES.md` Promoted section for full scope.
