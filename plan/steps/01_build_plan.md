...
- [ ] Phase 52 — Release process artifacts (CHANGELOG.md + RELEASING.md): 0.10.0 was tagged today with no in-repo release artefacts, and the mobile 0.7.0 → 0.10.0 upgrade doc had to be hand-reconstructed from `git log v0.7.0..v0.10.0` + per-phase briefs. Two units. Unit 1 — author `CHANGELOG.md` at the repo root following Keep-a-Changelog; back-fill entries for `0.2.0` through `0.10.0` from the phase briefs as the per-tag source (Added / Changed / Removed sections + "Migration notes" line for shapes that need consumer migration, e.g. `state.philosophicalAlignment` for 0.10.0 / Phase 42, `getCoastalMap` removal at iterate `b85f509`). Unit 2 — author `RELEASING.md` documenting the tag → `npm publish` flow (pre-release checklist, version bump + tag, publish step, post-publish CHANGELOG append). Both files cross-linked from `README.md`. Extend `scripts/deploy-check.mjs` with a "latest git tag matches CHANGELOG top heading" assertion. Promoted via `/oversight` 2026-05-19 (fourth oversight of the day) from `expand pass 9` candidate. Score 5×8/10 = 4.0.
