...
- [ ] Phase 75 — `previewTemplateAtRarity` helper (mobile item-library mod-visibility) per oversight-17 2026-05-23 promotion (selected to keep the loop productive post-Phase-74 + post-0.11.0; AUDIT bias `equipment/items` stays armed since this is the bias target — effective score 5.25). Closes the user-jot finding from `b5c8165` refined at oversight-15. New `previewTemplateAtRarity(templateId: string, rarity: ItemRarity, playerLevel: number, rng?: () => number): Equipment | undefined` helper on `src/Items/item.factory.ts` (or sibling); reuses the existing `dropItem` machinery with rng + rarity pinned; default `rng` = `() => 0.5` for reproducible per-(template, rarity, level) previews matching the Phase 70 Coastal Tyrant deterministic-drop pattern. Re-exported through `src/Items/index.ts` + top-level barrel (+1 runtime export — fixture 235 → 236). Two commit units: (1) engine helper + hermetic e2e at `src/Items/e2e/preview-template.engine.test.ts` covering returns-rolled-Equipment-by-rarity + deterministic-per-seed + undefined-for-unknown-templateId + requiredLevel-gated value-tier resolution; (2) docs — `docs/items.md` Modifier-catalogue section gains a "Previewing rolled mods (library / catalog views)" subsection naming the helper + deterministic-preview convention; `docs/api.md` Items block extended; `README.md` Items row extended; `CHANGELOG.md [unreleased] ### Added` entry. **No engine math change** — pure consumer-facing helper around the existing rollModifiers / resolveModifiers / dropItem pipeline. Mobile item-library view post-ship can call `previewTemplateAtRarity` per (template, rarity) cell to render the player-visible mod stack. Brief at dispatch picks between the two alternative shapes (`previewTemplateAtRarity → Equipment` vs `getTemplatePreviewModifiers → RolledModifier[]`); recommend the Equipment-returning shape since mobile already consumes the Equipment shape. Source: candidate filed at oversight-15 `077979e`; promoted at oversight-17 `<this-commit>`. Score 5×6/10 = 3.0 + 0.5 user-source + 1.5× items bias = **5.25**.
