...
- [ ] Phase 68 — Befriending mechanic v2: per-enemy `BefriendabilityConfig` (path (a) per oversight-11 2026-05-20 design pass). Adds an opt-in `Enemy.befriendabilityConfig?: BefriendabilityConfig` typed field; the simple Phase 36 mechanic ("both-defend caps `friendshipCounter`") stays as the default fallback when the field is absent. Brief at dispatch picks per-axis defaults; the spec'd shape carries `roundsThreshold?`, `hpGate?: { belowPct }`, `requiredStances?: PhilosophicalStance[]`, `requiredSkillUse?: string[]`, plus `defaultFallback?: 'both-defend-cap'` for explicit acks. The combat resolver / `endCombat` consults the config when present and falls through to Phase 36 semantics when not. Unblocks the Boss-tier befriendable enemy candidate (currently blocked-by this phase) — when this lands, that follow-up authors its config + reward in the same pass. Likely 3 commit units: (1) engine primitive (`BefriendabilityConfig` type + resolver wire-in + barrel + fixture bump + hermetic e2e); (2) author one boss-tier config (Coastal Tyrant — hpGate + mercy-stance gate per the design preview) as the first real-world authoring example to validate the shape; (3) docs + CHANGELOG (`docs/combat.md` Friendship Path subsection extension, `docs/enemy.md` BefriendabilityConfig column on the Befriendable enemies table, `[unreleased]` ### Added entry; `Knowledge-Gaps.md` Q5 follow-up note if applicable).
