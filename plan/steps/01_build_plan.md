...
- [ ] Phase 54 — Spec 05e (Set items) implementation: spec authored pre-loop but no engine/library/tests/docs ever shipped (only `Equipment.setMembership?: string` stub at `src/Items/types.ts:199`). 3-4 units. Unit 1 — `SetBonus` + `ItemSet` types in `src/Items/set.types.ts`; barrel exports through `src/Items/index.ts` + `src/index.ts`. Unit 2 — `getActiveSetBonuses(character): SetBonus[]` in `src/Items/set.engine.ts`; thread set bonuses through `initializeCombat` (combatStartTokens additive stacking per Spec 05e Goal) + `generateBasicActionResources` (set generationBonus entries). Unit 3 — 3 initial sets in `src/Items/set.library.ts` (Wanderer's Road is the Spec 05e reference). Unit 4 — hermetic e2e in `src/Items/e2e/sets.engine.test.ts` + `docs/equipment.md` "Set Items" section + tick the 8 acceptance boxes in `specs/05e-set-items.md`. Promoted via `/oversight` 2026-05-19 (same oversight as Phase 52 / 53) from the `[needs-user-call]` AUDIT row that corrective-drain iterate `eb807a0` surfaced. Supersedes the Spec 05e `[skipped]` row at `:26`. Score 4×6/10 = 2.4.
