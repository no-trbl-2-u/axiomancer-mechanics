...
- [ ] Phase 49 — Enemy-skill caster path (combat depth): refactor `executeSkill` to take a generic `caster: Combatant` + `target: Combatant` (today: positional `player + enemy` + `targetType` flag); extend `decideEnemyAction` to optionally pick a skill from the enemy's `equippedSkills` when affordable; author 1-2 elite/boss enemies with a real skill rotation; hermetic e2e drives an enemy-skill-fired round through `resolveCombatRound`. Pairs with the Phase 45 outlook-bias AI hook so the skill pick reads as in-character. Promoted via `/oversight` 2026-05-19. Score 6×4/10 = 2.4. See `plan/PHASE_CANDIDATES.md` "Phase 49 — Enemy-skill caster path".
