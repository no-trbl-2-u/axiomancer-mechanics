...
- [ ] Phase 70 — Boss-tier befriendable enemy (Coastal Tyrant content authoring) per oversight-13 2026-05-21 promotion. Content-only phase — all engine fields (Phase 60 + 62 + 68 + 69) shipped. Coastal Tyrant (`src/Enemy/enemy.library.ts:317`) carries the Phase 68 `befriendabilityConfig` predicate; this phase fills in the matching `friendshipReward`: 2-3 guaranteed items (consumables + one unique equipment piece — brief locks the specific unique at dispatch), `xpBonus: 50+` (boss-tier weight vs the normal-tier +10/+15), multi-paragraph narrative (magistrate-fallen-priest archetype matching `faith-pessimistic-transcendent`), `alignmentDelta` (thematic combined-axis shift inside the Phase 43 ±1..±5 band — brief picks values), `flagSet: 'befriended-coastal-tyrant'` (Phase 62 convention). Two commit units: (1) content authoring on enemy.library.ts; (2) hermetic e2e at `src/Game/e2e/befriend.engine.test.ts` driving the Phase 68 predicate set + validating all four `friendshipReward` thread paths (items / xpBonus / narrative / alignmentShift / flagSet) + `docs/enemy.md` Befriendable-enemies table flip (Coastal Tyrant row from "deferred" → fully-authored) + `CHANGELOG.md [unreleased] ### Changed` entry. Demonstrates the full Phase 60+62+68+69 stack on a single high-stakes encounter; pairs with Coastal Tyrant's existing Phase 49 skill rotation + Phase 45 alignment pin.
