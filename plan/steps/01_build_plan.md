...
- [ ] Phase 62 — Quest-branch wire-in on `outcome === 'friendship'` (Phase 60 D3 follow-up): promoted via oversight 2026-05-20 seventh-of-session. Pick path (a) — `FriendshipReward.flagSet?: string` + `store.endCombat()` dispatches a flag-set on friendship outcome; existing `DialogueChoice.requires.flag` machinery consumes downstream. Author one branching quest entry; e2e drives friendship → flag-set → dialogue-gate end-to-end. Best-paired with Phase 61 so the CHANGELOG `[unreleased]` block authored there folds the new `flagSet` field in. Score 5×5/10 = 2.5. See `plan/PHASE_CANDIDATES.md` Promoted section for full scope.
