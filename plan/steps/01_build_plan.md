...
- [ ] Phase 72 — Run-loop semantics (`resetRun` API + `runId` + starting-hearth) (mobile GH#65 ask 2, score 4.5) per oversight-14 2026-05-22 promotion. **Design-heavy — wants attended brief drafting at dispatch (multiple design directions on starting-hearth scope; save-migration shape).** Replaces the mobile BEGIN AGAIN full-heal band-aid (`state/combat-mode.tsx`) with a proper run-loop primitive on the engine. Scope: new `resetRun({ keepCharacter: boolean }): GameState` action on `src/Game/store.ts` (`keepCharacter=true` preserves persistent character — preset / skills / equipment / philosophicalAlignment; resets run-scoped state — currentMap / mapState / activeCombat / runtime flags / quest-progress flags), new `runId: string` field on `GameState` (UUID per run, bumped by `resetRun`, surfaced on save data for runs-history future work), and a starting-hearth concept (the canonical re-entry node when a run resets — likely `fv-1` for the fishing-village arc, but the type shape should let regions register their own). Save migration v? → v? defaults `runId` for legacy saves. Three commit units: (1) engine primitive + save migration; (2) starting-hearth registration + per-region wiring (default fishing-village hearth at fv-1; northern-forest hearth where authoring goes); (3) hermetic e2e (run-reset preserves character, clears run-state, increments runId; starting-hearth resolution) + `docs/api.md` Game block + `docs/gameloop.md` Run-loop subsection + `CHANGELOG.md [unreleased] ### Added`. Mobile callsite: `state/combat-mode.tsx` BEGIN AGAIN dispatches `resetRun({ keepCharacter: true })` instead of full-heal + dismiss. Source: GH#65 ask 2 (filed 2026-05-22, scored 4.5).
