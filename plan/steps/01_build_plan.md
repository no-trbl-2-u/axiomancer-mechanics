...
- [ ] Phase 37 — Shop economy via `village` MapEventKind: `buyItem(character, inventory, itemId, price)` + `sellItem(character, item, price)` reducers (pure — decrement / increment `Character.currency`, add / remove items). Wire into the `village` MapEventHandler so resolving a `village` node opens a shop UI in the Map tab. Author 1-2 starter shop inventories (reuse `consumableLibrary` for the demo pool). Hermetic e2e covers buy / sell / insufficient-funds. Makes `Character.currency` a meaningful resource; quest rewards that grant currency become spendable; `village` nodes are no longer flavour-only. Promoted via `/oversight` 2026-05-16.
- [ ] Phase 38 — `ActiveEffect.sourceId` wiring for player-applied effects: audit every `applyEffect` call site on the combat / skill path (`src/Skills/skill.engine.ts:447` rebound, `src/Combat/resist.ts:57/82/94` crit / rebound / overwhelmed, `src/Combat/effects.ts:113` extended buffs, `src/World/MapEvents/handlers.ts:163` map-event effects) and thread `sourceId` from the attacker's `id` (player) or `enemy.id`. Extend `ApplyEffectOptions` with `sourceId?: string`. Hermetic e2e pins that a player-applied DoT carries `sourceId === state.player.id` through save / load. Closes the Phase 35 attribution follow-up. Promoted via `/oversight` 2026-05-16.
- [ ] Phase 39 — Agent-friendly hermetic e2e report: custom Vitest reporter at `automation/agent-vitest-reporter.ts` that emits both a JSON report (`automation/last-verify-report.json` — rollup + per-file + per-test with failure messages, durations, slowest 5) and a delimited markdown summary on stdout (totals, per-file rollup, failed-assertion list with `file:line — message`, slowest 5). New additive `npm run verify:agent` script (existing `verify` untouched per Hard Rule 9 conservatism). Hermetic e2e drives the reporter against synthetic Vitest events to pin the JSON shape and markdown contents. Decisions captured upfront in `plan/PHASE_CANDIDATES.md`: audience = both, hook point = custom reporter (not post-process). Open Qs for `/plan-a-phase phase 39`: exact JSON schema, first call-out heuristic, markdown delimiter convention. Promoted via `/oversight` 2026-05-16.
