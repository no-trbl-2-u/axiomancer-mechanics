...
- [ ] Phase 61 — CHANGELOG breaking-removals backfill (mobile #93 unblock): promoted via oversight 2026-05-20 (seventh of session) after mobile #93 attempted the engine bump 0.10.0 → 0.10.2 and surfaced ~58 typecheck errors from undocumented breaking removals (`Encounter.enemy`, `DialogueChoice.id`/`.label`, `DialogueNode.speaker`, `Character.mana`/`.maxMana`, `ActiveEffect.id`/`.name`, `EffectStatTarget` literal-union tightening, `GameState` index signature removal, `getCoastalMap` already removed but mobile callers not migrated). Absorbs critique-26 (CHANGELOG `[unreleased]` empty). Scope: audit each tag `v0.7.0` → `v0.10.2` via `dist/index.d.ts` diff + per-phase briefs; populate `### Removed` / `### Breaking` blocks where applicable; populate `[unreleased]` with Phase 58/59/60 + post-0.10.2 iterate fixes; optional Unit 3 comments on mobile #93 with the canonical migration guide. Score 7×8/10 = 5.6. Closes critique-26; unblocks mobile #93 Phase 60 re-scope.
