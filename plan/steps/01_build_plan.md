...
- [ ] Phase 36 — Friendship victory reward (XP + narrative tag): route the friendship-counter exit through a `'friendship'` outcome in `determineCombatEnd` (currently reports `'flee'`); have `endCombat` grant `floor(enemy.xpReward * 0.5)` XP on that path; surface `outcome: 'friendship'` on the `combat:ended` event payload; CLI transcript distinguishes "befriended" from "fled". Closes Knowledge-Gaps Q5 + Spec 06 Q2 follow-up. Promoted via `/oversight` 2026-05-15.
