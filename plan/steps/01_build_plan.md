...
- [ ] Phase 65 — Expand the fishing-village starting map ("huge first map" pass): promoted via oversight 2026-05-20 (eighth-of-session) as a user write-in. Current `fishing-village` is a linear 10-node chain (`fv-1` → `fv-10`); the user wants it considerably larger as the canonical "base" map until later balancing phases break it up. Target: ~25-35 nodes in a 2D branching grid with sub-areas (harbor district / inland streets / cliff path / etc.), 3-5 dead-end / loop branches. Author MapEventPool entries per the Phase 23 8-kind taxonomy (encounter / interaction / gathering / rest / village / cutscene / hazard / loot-cache). Reuse existing enemies + NPCs (no new enemy / NPC content this phase). Phase brief at dispatch picks final node count + layout + per-node assignment. Score 7×5/10 = 3.5. See `plan/PHASE_CANDIDATES.md` Promoted section for full scope.
