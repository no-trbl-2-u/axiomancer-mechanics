...
- [ ] Phase 74 — Post-GH#65 per-foe content sweep (Phase 71 + Phase 73 missing-enemy authoring) per oversight-16 2026-05-23 promotion (selected to refill the queue post-GH#65 trio drain saturation). Authors `finalBlowLines` + `causeLines` (and where the brief picks: `pactLines` + `journalEntry` + `friendshipReward`) on the 13 non-befriendable enemies in `src/Enemy/enemy.library.ts`: TidepoolCrab, SeaMistWisp, LullabyMoth, Disatree_01, WetHound, ForestSprite, ArgumentativeCrow, TideflukeReaver, HushWraith, HollowSaint, TheDisagreement, EchoOfPyrrhonia, Sandbag_01. Three commit units: (1) finalBlowLines + causeLines on the 13 — 6 strings per enemy × 13 = 78 chronicle lines, voice continues from each enemy's existing `description` + Phase 45 alignment pin + Phase 49 skill rotation where present; Sandbag_01 is a test sandbox so author trivial placeholder or skip per brief discretion. (2) Optional newly-befriendable enemy authoring — brief at dispatch picks 1-2 candidates from the 13 that thematically suit a befriend arc (TideflukeReaver, HushWraith, TheDisagreement are plausible Tier-2; HollowSaint is plausible boss-tier); each new befriendable enemy gets the full Phase 60/62/68/69/71/73 stack (`friendshipReward` + `flagSet` + `befriendabilityConfig` + `alignmentDelta` + `pactLines` + `journalEntry`). If the brief decides nothing else befriendable lands, Unit 2 collapses. (3) Extend the Phase 71 + Phase 73 hermetic e2e files with registration assertions for the newly-authored enemies; extend `docs/enemy.md` Aftermath narrative + Codex sections with the new coverage; `CHANGELOG.md [unreleased] ### Added` entry naming the per-foe additions. **No engine change** — pure content authoring against the fully-shipped GH#65 trio engine surface. Mobile aftermath panel will render engine-authored chronicle prose for every encounter post-ship (no presenter-derived fallback for the player-visible majority of fights). Source: candidate filed at expand-20 `f892f3a`; promoted at oversight-16 `<this-commit>`. Score 3.0.
