...
- [ ] Phase 56 — CI verify-on-PR / verify-on-push (GitHub Actions): closes the PR-level CI gap for the published-npm package. 1-2 units. Unit 1 — author `.github/workflows/verify.yml` running on `pull_request` against main + `push` against main. Steps: Node setup (LTS), `npm ci`, `npm run verify` (type-check + tests + build), `npm run deploy:check`. Cache `node_modules` keyed off `package-lock.json` sha. Unit 2 (optional) — release workflow on tag push (`v*.*.*`) that runs verify + builds dist + prepares a draft `npm publish` (without auto-publishing per RELEASING.md). Future human contributors / branch experiments get a remote red-CI signal; README can carry a CI badge. Promoted via `/oversight` 2026-05-20 (same oversight as Phase 55) from `expand pass 13` candidate. Score 4×7/10 = 2.8.
