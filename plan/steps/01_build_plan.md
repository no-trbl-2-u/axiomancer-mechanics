...
- [ ] Phase 69 — `FriendshipReward.alignmentDelta` extension (closes Spec 14 Q4) per oversight-12 2026-05-21 promotion. Adds `alignmentDelta?: Partial<PhilosophicalAlignment>` to the `FriendshipReward` interface; `store.endCombat()` threads the delta through `applyAlignmentDelta` on the `outcome === 'friendship'` branch (after the existing items / xpBonus / narrative / flagSet threads); `CombatEndReport.friendshipReward` gains an optional `alignmentShift?: PhilosophicalAlignment` surfacing the post-clamp pin. Two commit units: (1) engine primitive + hermetic e2e at `src/Game/e2e/befriend.engine.test.ts` (cold-cell + clamp-edge cases) + fixture bump 162 → 163 types + barrel re-exports; (2) content authoring (MournfulGull + HollowEyedBeggar `alignmentDelta` values locked at brief time) + `docs/enemy.md` Befriendable-enemies table extension + `specs/14-philosophical-alignment.md` Q4 flipped to "Resolved at Phase 69" + `CHANGELOG.md [unreleased] ### Added` entry + `plan/bearings.md` Enemy block update. Pairs with Phase 43's `applyAlignmentDelta` clamp helper as the canonical reference. Unblocks the Boss-tier befriendable enemy candidate's `alignmentDelta` sub-scope; its remaining scope shrinks to narrative + items only.
