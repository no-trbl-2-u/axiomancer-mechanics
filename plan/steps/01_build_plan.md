...
- [ ] Phase 46 — Alignment-gated content (`requires.alignment` on dialogue + skill learning): promoted via `/oversight` 2026-05-16 (promote-multiple sequence 46 → 47 → 48). Three units. Unit 1 — extend `DialogueChoice.requires` with `requiresAlignment?: { axis: 'epistemology'|'outlook'|'scope', op: 'gte'|'lte', value: number }`; `visibleChoices` filters by player's `philosophicalAlignment`. Unit 2 — extend `SkillLearningRequirement` with the same shape; `meetsLearningRequirement` checks it before allowing `learnSkill`. Unit 3 — author 2-3 alignment-gated dialogue branches on Old Marrow / Coastal Beggar + 1-2 alignment-gated Tier 3 skills (e.g. `appeal-to-fear` only learnable at `scope > 33`); hermetic e2e drives each gate path; update `docs/philosophy.md` "Authoring gates" subsection. Closes the Phase 42 Follow-up "Alignment-gated skills / effects / endings". Score 6×5/10 = 3.0.
- [ ] Phase 47 — Knowledge-Gaps acceptance sweep: promoted via `/oversight` 2026-05-16 (promote-multiple). Mirrors Phase 41's spec-sweep pattern. Walk every numbered question in `Knowledge-Gaps.md`, classify each into (a) deliberately deferred combat-tuning (Q1/Q2/Q4/Q6), (b) resolved by shipped phases but never marked (~Q8-Q11, Q13, Q14, Q16, Q22-Q25), (c) genuinely open + actionable (e.g. Q14 cap, Q16 max slots — flag for /oversight or file as iterate rows). For (b) entries, append "**Resolved at <spec / phase> (<hash>).**" Pure docs work; 3-4 commit units. Score 4×8/10 = 3.2.
- [ ] Phase 48 — Effect runtime: apply `statModifiers` + intensity scaling to derived stats (KG Q8/Q9): promoted via `/oversight` 2026-05-16 (promote-multiple). Two units. Unit 1 — extend / add `getEffectiveStats(character)` to fold every active effect's `statModifiers[]` into derived stats, scaled by intensity when an optional `intensityScalesStatModifiers?: boolean` is set on the Effect; thread every `derivedStats` read site through the helper. Unit 2 — hermetic e2e for buff stat-up + debuff stat-down + intensity scaling; update `docs/effects.md` Payload Field Reference. Closes KG Q8 + Q9 and activates the ~66 `statModifiers`-bearing effects already authored in the library (including Phase 44's `debuff_no_true_scotsman`). Score 6×5/10 = 3.0.
