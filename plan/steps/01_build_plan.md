...
- [ ] Phase 57 — Enemy rotation content sweep (Phase 49 follow-up): Phase 49 shipped the enemy-skill caster path but only authored 2 rotations (Argumentative Crow → `false-dilemma`, Coastal Tyrant → `achilles-gambit`). 14 remaining authored enemies in `src/Enemy/enemy.library.ts` have empty `Enemy.skills?`. 2 units. Unit 1 — author one thematic skill per remaining elite (Tidefluke Reaver, Hush-Wraith, Hollow Saint) + remaining boss (The Disagreement) + unique (Echo of Pyrrhonia); alignment cell (Phase 45) guides the pick (e.g. Hollow Saint faith-mid-transcendent → `pascals-wager`). Unit 2 — author rotations for 2-3 selected normal enemies (Mournful Gull → `appeal-to-pity`, Hollow-Eyed Beggar → `pascals-wager`); leave simple enemies (Tidepool Crab, Sea-Mist Wisp, Lullaby Moth) without skills since early-game pacing benefits from straight basic-action enemies. Hermetic e2e extends the existing Phase 49 rotation tests; `docs/enemy.md` Phase 49 subsection table grows. Promoted via `/oversight` 2026-05-20 (same oversight as Phase 55 + 56) from `expand pass 8` candidate. Score 5×7/10 = 3.5.
