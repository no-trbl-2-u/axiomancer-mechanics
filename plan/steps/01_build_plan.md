...
- [ ] Phase 50 — Engine handoff for `axiomancer-mobile` (skillLibrary re-export + types.d.ts emission fix): unblocks 3 mobile phases gated on `axiomancer-mechanics` release per `axiomancer-mobile/docs/engine-team-handoff-2026-05-16.md`. Two units. Unit 1 — add `export { skillLibrary, getSkillById } from './Skills';` to `src/index.ts` Skills block + hermetic public-barrel-import test in `src/test-utils/e2e/`. Unit 2 — investigate why `types.d.ts` is missing from 6 of 9 `dist/` sub-paths (Combat / Effects / Skills / Game top-level / Character / Enemy / NPCs / World top-level); compare working sub-paths' tsconfig against missing ones, apply the fix; extend `scripts/deploy-check.mjs` (or add a smoke assertion) so the count regression can't recur silently. Version bump + npm publish lands as a separate user-triggered step. Promoted via `/oversight` 2026-05-19. Score 6×7/10 = 4.2.
- [ ] Phase 51 — Autosave throttling per Spec 09 Q4: drain the standing `TODO(spec-09)` comments at `src/Game/store.ts:203` + `src/Game/game.reducer.ts:138`. Pick (B) restrict-by-action-type (recommended in candidate scope) — only autosave on a curated set (`COMBAT_ROUND`, `LEVEL_UP`, `END_COMBAT`, `MOVE_TO_NODE`, `APPLY_DIALOGUE`, `SAVE_GAME`); UI-tier actions never trigger writes. Hermetic e2e drives the store through a series of actions and counts `adapter.save` invocations. Pre-emptive readiness for the React Native AsyncStorage backend. Drains two standing TODOs + closes Spec 09 Q4. Promoted via `/oversight` 2026-05-19. Score 4×6/10 = 2.4.
