...
| Hazard-Pattern Combat (Spec 25) | Card-and-dice combat engine that ships **alongside** `resolveCombatRound` — every verb is a skill card, and status effects fill two Pressure Tracks (DoT Erosion + Control Saturation) that are the only practical win conditions (basic-attack trading is gone — the doctrine path is now the structural path). Drive a fight with `initializeCombatEncounter`, `rollEncounterDice`, `playCombatCard`, `resolveCombatPhase`/`resolveThreatPhase`/`processBetweenPhases`, `buildCombatSummary`; preview helpers `getCard`/`handCards`/`cardDieCostPreview`/`availableDice`; dice/deck/card/pressure/threat sub-modules (`rollCombatDice`, `buildCombatDeck`, `toCombatCard`, `dotErosionReached`/`controlSaturationReached`, `getThreatSequence`); Monte-Carlo greedy bot `simulateHazardPatternCombat`. Types: `CombatEncounterState`, `CombatCard`, `CombatPressureTracks`, `CombatThreatPhase`, `CombatOutcome`, `CombatSummary`, `CombatSimStats`. See [`docs/combat.md`](./docs/combat.md) → Hazard-Pattern Combat. |
