/**
 * Deterministic RNG for the Loot-cache encounter engine (mulberry32).
 *
 * Sibling copy of the Hazard/Gathering/QuestBoard RNG so the
 * `World/LootCache/` directory stays self-contained. The engine threads
...
export interface LootCacheRngState {
    /** mulberry32 internal state word (uint32). */
    s: number;
}

export function seedRng(seed: SeedInput): LootCacheRngState {
...
export function nextFloat(state: LootCacheRngState): { value: number; state: LootCacheRngState } {
...
export function nextInt(state: LootCacheRngState, n: number): { value: number; state: LootCacheRngState } {
    const { value, state: next } = nextFloat(state);
    return { value: Math.floor(value * n), state: next };
}

/** Rolls a d6: an int in [1, 6]. Pure. */
export function rollDie(state: LootCacheRngState): { value: number; state: LootCacheRngState } {
...
export function shuffle<T>(
    state: LootCacheRngState,
    items: readonly T[],
): { value: T[]; state: LootCacheRngState } {
